...
PATCH=Path('/Users/mac/code/my/AutoTeam-F/.open-reverselab/patches/openai-paypal-main-us-country-schema-applyable.diff')
WORK=Path('/tmp/openai-paypal-main-us-schema-acceptance')
PYTHON=SRC/'.venv/bin/python'

def copy_clean():
    if WORK.exists(): shutil.rmtree(WORK)
    def ignore(_dir, names):
        return {n for n in names if n in {'.venv','__pycache__','.git','var','cache'}}
...
def main():
    copy_clean()
    # diff was generated against paypal/ absolute dirs; apply with -p4 maps to paypal/*.
    r=run(['patch','-p0','--batch','--forward','-i',str(PATCH)])
    if r.returncode not in (0,1):
        print(json.dumps({'ok':False,'stage':'patch','returncode':r.returncode,'output':r.stdout[-2000:]}, indent=2)); return 1
    compile_cmd=[str(PYTHON),'-m','py_compile','paypal/country_profile.py','paypal/flow.py','paypal/session.py']
    r=run(compile_cmd)
    if r.returncode != 0:
        print(json.dumps({'ok':False,'stage':'compile','output':r.stdout[-4000:]}, indent=2)); return 1
    probe=WORK/'_probe_us_payload.py'
    probe.write_text('''
import json, sys
...
print(json.dumps({
 ...
 'contentIdentifier':v.get('contentIdentifier'),
 'phone':v.get('phone'),
}, indent=2))
''')
    r=run([str(PYTHON),str(probe)])
    if r.returncode != 0:
        print(json.dumps({'ok':False,'stage':'probe','output':r.stdout[-4000:]}, indent=2)); return 1
    text=r.stdout
    start=text.find('{')
    ...
    print(json.dumps({'ok':ok,'stage':'us_schema_payload','checks':checks,'probe':data}, ensure_ascii=False, indent=2))
