...
import { deleteAuthFileFromCLIProxyAPI, downloadAuthFileJsonObjectFromCLIProxyAPI, listAuthFilesFromCLIProxyAPI, saveAuthFileJsonObjectToCLIProxyAPI, setAuthFileDisabledStatusToCLIProxyAPI, } from "./cliproxyapi.js";
...
async function collectAuthFiles(rootDir) {
...
async function collectCPAAuthTargets() {
    ...
    const targets = files
        ...
        return {
            ...
            async saveRecord(record) {
                await saveAuthFileJsonObjectToCLIProxyAPI(name, record);
            },
            async moveTo401() {
...
function extractMessage(rawBody) {
...
function shouldMoveTo401(message) {
...
async function sendUsageProbe(accessToken, accountId) {
...
function normalizeRefreshedAuthRecord(existing, payload) {
    ...
    const accountId = accessClaims?.["https://api.openai.com/auth"]?.chatgpt_account_id?.trim() ||
...
async function saveAuthRecord(filePath, record) {
...
function printTable(rows) {
    ...
    const widths = {
        ...
        limitReached: Math.max(headers.limitReached.length, ...rows.map((row) => row.limitReached.length)),
...
async function moveTo401Dir(filePath) {
...
function resolveConcurrency(total) {
...
async function summarizeAuth(filePath, forceRefresh) {
...
async function summarizeAuthTarget(target, forceRefresh) {
    ...
    return {
        ...
        limitReached: typeof payload?.rate_limit?.limit_reached === "boolean"
...
async function main() {
    ...
    const targetItems = Number.isFinite(limitArg) && limitArg > 0 ? targets.slice(0, limitArg) : targets;
    ...
    console.log(`准备检查 ${targetItems.length} 个 auth 文件: ${useCPA ? "CPA" : authDir}${forceRefresh ? " (强制刷新 token)" : ""} (并发: ${concurrency})`);
    const indexedRows = await mapWithConcurrency(targetItems, concurrency, async (target, index) => {
    ...
    console.log(`总数 ${totalCount} | 可用 ${availableCount} | 限额 ${limitedCount} | 移除 ${movedCount} | 可用额度 ${totalRemaining.toFixed(2)}`);
    if (hasFlag("--table")) {
        printTable(rows);
    }
}
