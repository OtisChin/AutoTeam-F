...
function loadConfig() {
    ...
    return {
        ...
        defaultPassword: typeof parsed.defaultPassword === "string" && parsed.defaultPassword.trim()
        ...
        gmailAccessToken: typeof parsed.gmailAccessToken === "string"
            ? parsed.gmailAccessToken.trim()
            : DEFAULT_CONFIG.gmailAccessToken,
        gmailEmailAddress: typeof parsed.gmailEmailAddress === "string"
            ? parsed.gmailEmailAddress.trim()
            : DEFAULT_CONFIG.gmailEmailAddress,
        gptMailApiKey: typeof parsed.gptMailApiKey === "string"
            ? parsed.gptMailApiKey.trim()
            : DEFAULT_CONFIG.gptMailApiKey,
        gptMailDomain: typeof parsed.gptMailDomain === "string"
            ? parsed.gptMailDomain.trim()
            : DEFAULT_CONFIG.gptMailDomain,
        "2925EmailAddress": typeof parsed["2925EmailAddress"] === "string"
            ? parsed["2925EmailAddress"].trim()
            : DEFAULT_CONFIG["2925EmailAddress"],
        "2925Password": typeof parsed["2925Password"] === "string"
            ? parsed["2925Password"].trim()
            : DEFAULT_CONFIG["2925Password"],
        cloudflareEmailDomain: typeof parsed.cloudflareEmailDomain === "string" && parsed.cloudflareEmailDomain.trim()
            ? parsed.cloudflareEmailDomain.trim()
            : DEFAULT_CONFIG.cloudflareEmailDomain,
        cloudflareApiBaseUrl: typeof parsed.cloudflareApiBaseUrl === "string"
            ? parsed.cloudflareApiBaseUrl.trim()
            : DEFAULT_CONFIG.cloudflareApiBaseUrl,
        cloudflareApiKey: typeof parsed.cloudflareApiKey === "string"
            ? parsed.cloudflareApiKey.trim()
            : DEFAULT_CONFIG.cloudflareApiKey,
        defaultProxyUrl: typeof parsed.defaultProxyUrl === "string"
            ? parsed.defaultProxyUrl.trim()
            : DEFAULT_CONFIG.defaultProxyUrl,
        heroSMSApiKey: typeof parsed.heroSMSApiKey === "string"
            ? parsed.heroSMSApiKey.trim()
            : DEFAULT_CONFIG.heroSMSApiKey,
        heroSMSCountry: typeof parsed.heroSMSCountry === "number"
            ? parsed.heroSMSCountry
            : DEFAULT_CONFIG.heroSMSCountry,
        heroSMSMaxPrice: typeof parsed.heroSMSMaxPrice === "number"
            ? parsed.heroSMSMaxPrice
            : DEFAULT_CONFIG.heroSMSMaxPrice,
        heroSMSPriceTiers: Array.isArray(parsed.heroSMSPriceTiers)
            ? parsed.heroSMSPriceTiers
                .map((v) => Number(v))
                .filter((v) => Number.isFinite(v) && v > 0)
            : undefined,
        heroSMSPollAttempts: typeof parsed.heroSMSPollAttempts === "number"
            ? parsed.heroSMSPollAttempts
            : DEFAULT_CONFIG.heroSMSPollAttempts,
        heroSMSPollIntervalMs: typeof parsed.heroSMSPollIntervalMs === "number"
            ? parsed.heroSMSPollIntervalMs
            : DEFAULT_CONFIG.heroSMSPollIntervalMs,
        cliproxyApiAutoUploadAuth: normalizeBoolean(parsed.cliproxyApiAutoUploadAuth, DEFAULT_CONFIG.cliproxyApiAutoUploadAuth),
        cliproxyApiBaseUrl: typeof parsed.cliproxyApiBaseUrl === "string" && parsed.cliproxyApiBaseUrl.trim()
            ? parsed.cliproxyApiBaseUrl.trim()
            : DEFAULT_CONFIG.cliproxyApiBaseUrl,
        cliproxyApiManagementKey: typeof parsed.cliproxyApiManagementKey === "string"
