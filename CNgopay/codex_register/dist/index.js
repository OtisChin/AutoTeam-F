...
async function runOnce() {
    ...
    if (codexCpa) {
        ...
        let fetchAddEmailOtp = undefined;
        ...
        if (!bindEmail) {
            throw new Error(`缺少 add-email 绑定邮箱：请用 --bind-email 指定，或配置可用的 hotmail 卡密池` +
                `（hotmail/tokens.txt 或 HOTMAIL_TOKENS_FILE 指向的文件）。` +
                (hotmailPrepError ? ` hotmail 准备失败原因: ${hotmailPrepError}` : ``));
        }
        if (!fetchAddEmailOtp) {
            throw new Error(`已有 bindEmail=${bindEmail} 但无法接收 add-email OTP：hotmail provider 初始化失败` +
        ...
        if (phoneArg) {
            ...
            for (let phoneTry = 1; phoneTry <= MAX_PHONE_TRIES; phoneTry += 1) {
                ...
                try {
                    ...
                    lastErr = e;
                    console.warn(`[codex-cpa] (${phoneTry}/${MAX_PHONE_TRIES}) 失败: ${e.message}`);
                    try {
        ...
        try {
            ...
            if (msg.includes("email_already_in_use") && bindEmail) {
                ...
                try {
                    ...
                    console.warn(`[codex-cpa] [hotmail 卡密消费失败] ${consumeErr.message}`);
                }
            }
            throw e;
        }
        ...
        if (!chatgptAccessToken) {
            try {
                ...
                if (!latest) {
                    throw new Error(`等了 ${POLL_MAX_ATTEMPTS * POLL_INTERVAL_MS}ms 仍找不到 codex-${emailLc}(.json|-plus.json)`
                ...
                console.warn(`[codex-cpa] 从 CPA 取 access_token 失败: ${e.message}`);
            }
        }
        ...
        if (chatgptAccessToken) {
            ...
            if (probeJP) {
                try {
                    ...
                    if (probeRes.hasTrial) {
                        ...
                        if (bindEmail) {
                            try {
                                ...
                                console.warn(`[codex-cpa] [hotmail 卡密消费失败，忽略] ${e.message}`);
                            }
                        }
                    ...
                    console.warn(`[codex-cpa] [试用探测失败，继续主流程] ${probeErr.message}`);
                }
            }
            else {
            ...
            if (!gpTokenFile) {
                ...
                try {
                    ...
                    console.warn(`[codex-cpa] 写 token 文件失败: ${e.message}`);
                }
            }
        }
        else {
    ...
    if (phoneFirst) {
        ...
        // 所以我们改走 codex CLI 的 OAuth 流程（账号已经在 OpenAI 后端创建好了，
        // 现在用同 phone+password 登录即可）。
        console.log(`[phone-signup] 切换到 codex CLI OAuth 流程拿 token...`);
        ...
        try {
            ...
            console.warn(`[phone-signup] hotmail 邮箱准备失败 (无 add-email 兜底): ${e.message}`);
        }
        ...
        try {
            ...
            console.warn(`[phone-signup] codex OAuth 登录失败: ${e.message}`);
            // 不抛错，继续尝试拿 ChatGPT plan token
        }
        // 拿 ChatGPT plan accessToken（用 reauth 后的 cookie）
        let chatgptAccessToken = "";
        try {
            chatgptAccessToken = await reauthClient.getChatGPTAccessToken();
        }
        catch (err) {
            console.warn(`[警告] reauth 拿 ChatGPT accessToken 失败: ${err.message}`);
            // 最后兜底：用第一个 client 试
            try {
                chatgptAccessToken = await client.getChatGPTAccessToken();
            }
            catch (err2) {
                console.warn(`[警告] 主 client 拿也失败: ${err2.message}`);
            }
        }
        if (!chatgptAccessToken) {
    ...
    if (directSignupAuth && saveAccessToken) {
        ...
        try {
            ...
            console.warn(`[警告] 拿 ChatGPT accessToken 失败 (${err.message})，尝试重新登录`);
            const reauthClient = new OpenAIClient({
                email: client.email,
                password: appConfig.defaultPassword,
                deviceProfile: generateRandomDeviceProfile(),
            ...
            try {
                ...
                console.warn(`[警告] 重登录失败: ${loginErr.message}`);
            }
            chatgptAccessToken = await reauthClient.getChatGPTAccessToken();
        }
        ...
        if (gpTokenOutPath) {
            try {
                ...
                console.warn(`[警告] 写 gp-token-out 失败: ${e.message}`);
            }
        }
        return;
    }
    ...
    if (saveAccessToken) {
        ...
        try {
            ...
            console.warn(`[警告] 注册后直接拿 accessToken 失败 (${err.message})，尝试重新登录`);
            ...
            try {
                ...
                console.warn(`[警告] 重登录也失败: ${loginErr.message}`);
            }
...
async function main() {
        ...
        try {
            ...
            if (hasFlag("--at")) {
                ...
                try {
                    ...
                    console.warn(`[警告] 拿 ChatGPT accessToken 失败 (${err.message})`);
                }
                if (chatgptAccessToken) {
                    const accessTokenFile = await client.saveChatGPTAccessToken(chatgptAccessToken);
                    console.log(`[access_token_file] ${accessTokenFile}`);
    ...
    while (!maxRounds || round < maxRounds) {
        round += 1;
        console.log(`第 ${round} 轮开始: 成功=${successCount} 失败=${failCount} 模式=自动`);
        try {
            await runOnce();
            successCount += 1;
        }
    ...
    console.log(`自动模式结束: 已执行=${round} 成功=${successCount} 失败=${failCount}`);
}
main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
