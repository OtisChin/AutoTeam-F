...
import { findLatestVerificationMail as findLatestVerificationMailByFields, normalizeMailbox, } from "./verification-matcher.js";
...
const MAIL_LIST_URL = "https://www.2925.com/mailv2/maildata/MailList/mails";
const MAIL_READ_URL = "https://www.2925.com/mailv2/maildata/MailRead/mails/read";
const MOVE_MAILS_URL = "https://www.2925.com/mailv2/maildata/MailData/mails/folder";
const SESSION_CACHE_DIR = process.cwd();
const SESSION_CACHE_FILE = path.join(SESSION_CACHE_DIR, "2925-account.json");
const MOBILE_USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1 Edg/146.0.0.0";
...
function findLatestVerificationMail(mails, options = {}) {
    ...
    return findLatestVerificationMailByFields(mails.map((mail) => ({
        ...mail,
...
async function fetchLatestVerificationCode(options) {
    ...
    if (!latestMail) {
        ...
        for (const mail of sorted) {
            if (normalizedTarget) {
                const matchedRecipient = mail.toAddress.some((address) => normalizeMailbox(address) === normalizedTarget);
                if (!matchedRecipient) {
                    continue;
                }
            }
    ...
    return {
        ...
        async getEmailVerificationCode(email) {
            for (let attempt = 1; attempt <= PROVIDER_POLL_ATTEMPTS; attempt += 1) {
                console.log(`poll2925Otp: attempt=${attempt}/${PROVIDER_POLL_ATTEMPTS} targetEmail=${email}`);
                const result = await fetchLatestVerificationCodeWithSession(email);
                if (result.code) {
                    console.log(`2925OtpCode: ${result.code}`);
                    console.log(`autoEmailOtpDeletedCount: ${result.deleteResult?.count ?? 0}`);
                    return result.code;
                }
