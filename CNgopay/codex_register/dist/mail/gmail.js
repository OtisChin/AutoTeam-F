...
async function getMessage(messageId) {
    const payload = await gmailRequest(`/users/${encodeURIComponent(GMAIL_USER_ID)}/messages/${encodeURIComponent(messageId)}`, {
        format: "full",
    });
...
async function deleteMessage(messageId) {
    await gmailDeleteRequest(`/users/${encodeURIComponent(GMAIL_USER_ID)}/messages/${encodeURIComponent(messageId)}`);
}
    ...
    return {
        ...
        async getEmailVerificationCode(email) {
            for (let attempt = 1; attempt <= GMAIL_POLL_ATTEMPTS; attempt += 1) {
                console.log(`pollGmailOtp: attempt=${attempt}/${GMAIL_POLL_ATTEMPTS} targetEmail=${email}`);
                ...
                if (attempt < GMAIL_POLL_ATTEMPTS) {
                    await new Promise((resolve) => setTimeout(resolve, GMAIL_POLL_INTERVAL_MS));
                }
            }
