    ...
    for (const variant of buildRefreshVariants(account)) {
        ...
        for (let attempt = 1; attempt <= 4; attempt += 1) {
            try {
                ...
                console.warn(`hotmailTokenRefreshNetErr: attempt=${attempt}/4 ${err.message}, ${wait}ms 后重试`);
                await new Promise((r) => setTimeout(r, wait));
            }
        }
        if (!response) {
            lastError = `network: ${networkErr?.message ?? "unknown"}`;
            continue;
        }
...
async function restRequest(account, url) {
    ...
    if (!response) {
        throw new Error(`Hotmail REST 网络错误: ${lastErr?.message ?? "unknown"}`);
    }
...
async function graphRequest(account, url) {
    ...
    if (!response) {
        throw new Error(`Hotmail Graph 网络错误: ${lastErr?.message ?? "unknown"}`);
    }
    if (response.status === 401) {
        await refreshAccessToken(account);
        response = await fetch(url, {
...
async function getLatestVerificationMessage(targetEmail, account, minTimestampMs = 0) {
    ...
    const debugLines = filtered.slice(0, 5).map((m) => `  [${m.folderId}] from=${m.from} to=${(m.toRecipients ?? []).join(',')} subject=${(m.subject ?? '').slice(0, 80)} bodyLen=${(m.bodyPreview ?? '').length} time=${new Date(m.receivedAtMs ?? 0).toISOString()}`);
    ...
    return findLatestVerificationMail(filtered.map((message) => ({
        ...message,
        ...
        targetEmail,
        candidateMatcher: (mail) => /(OpenAI|ChatGPT)/i.test(`${mail.subject ?? ""}\n${mail.bodyPreview ?? ""}\n${mail.from ?? ""}`),
    });
}
    ...
    return {
        ...
        async getEmailVerificationCode(email, options) {
            ...
            for (let attempt = 1; attempt <= HOTMAIL_POLL_ATTEMPTS; attempt += 1) {
                console.log(`pollHotmailOtp: attempt=${attempt}/${HOTMAIL_POLL_ATTEMPTS} targetEmail=${email} mailbox=${account.loginHint}`);
                try {
                    ...
                    console.warn(`pollHotmailOtp: attempt=${attempt} 读邮件失败: ${err.message}`);
                }
