...
async function persistAccountState() {
    await writeFile(PROXIEDMAIL_ACCOUNT_FILE, `${JSON.stringify({
...
async function resolveProxyBindingByAddress(email) {
    ...
    const match = bindings.find((item) => String(item?.attributes?.proxy_address ?? "").toLowerCase() ===
...
async function listReceivedEmailLinks(proxyBindingId) {
    const payload = await requestJSON(`/received-emails-links/${encodeURIComponent(proxyBindingId)}`, {
        method: "GET",
    });
    return Array.isArray(payload?.data) ? payload.data : [];
}
async function getReceivedEmail(receivedEmailId) {
    const payload = await requestJSON(`/received-emails/${encodeURIComponent(receivedEmailId)}`, {
        method: "GET",
    });
...
async function getLatestVerificationMessage(email, options = {}) {
    ...
    for (const item of links) {
        ...
        const receivedEmailId = id || linkPath.split("/").filter(Boolean).at(-1) || "";
        if (!receivedEmailId) {
            continue;
        }
    ...
    return {
        ...
        async getEmailVerificationCode(email) {
            ...
            for (let attempt = 1; attempt <= PROXIEDMAIL_POLL_ATTEMPTS; attempt += 1) {
                console.log(`pollProxiedMailOtp: attempt=${attempt}/${PROXIEDMAIL_POLL_ATTEMPTS} targetEmail=${email}`);
                ...
                if (attempt < PROXIEDMAIL_POLL_ATTEMPTS) {
                    await new Promise((resolve) => setTimeout(resolve, PROXIEDMAIL_POLL_INTERVAL_MS));
                }
            }
