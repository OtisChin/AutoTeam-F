...
import { AUTH_AUTHORIZE_CONTINUE_URL, AUTH_BASE_URL, AUTH_EMAIL_OTP_SEND_URL, AUTH_EMAIL_OTP_VALIDATE_URL, AUTH_OAUTH_TOKEN_URLS, AUTH_PASSWORD_VERIFY_URL, AUTH_REGISTER_URL, AUTH_WORKSPACE_SELECT_URL, CHATGPT_BASE_URL, DEFAULT_CLIENT_ID, DEFAULT_REDIRECT_URI, DEFAULT_USER_AGENT, } from "./constants.js";
...
function createDispatcher(proxyUrl, allowInsecureTLS) {
    if (!proxyUrl) {
        return new Agent({
            connect: {
...
async function createSocksSocket(proxyUrl, options, allowInsecureTLS) {
    const destinationHost = String(options.hostname ?? "");
    const rawPort = options.port;
    const destinationPort = rawPort === "" || rawPort == null
...
export class OpenAIClient {
    ...
    constructor(options) {
        ...
        const cookieFetch = makeFetchCookie(fetch, this.jar);
        this.fetch = ((input, init) => this.fetchWithRetry(cookieFetch, input, init));
    }
     ...
     */
    async authLoginViaCpaAuthorizeURL(authorizeUrl) {
        const totalSteps = 6;
        ...
        if (continueURL === `${AUTH_BASE_URL}/add-email`) {
            ...
            if (continueURL === `${AUTH_BASE_URL}/email-verification`) {
                ...
                if (!code)
                    throw new Error("add-email OTP 未提供");
     ...
     */
    async followToLocalhostCallback(startURL) {
        ...
        if (continueURL === `${AUTH_BASE_URL}/add-phone`) {
            ...
            for (let phoneTry = 1; phoneTry <= MAX_PHONE_ATTEMPTS; phoneTry += 1) {
                ...
                try {
                    ...
                    lastErr = e;
                    console.warn(`[add-phone] sendPhoneOtp 失败(${e.message}), 换号重试`);
                ...
                try {
                    ...
                    lastErr = e;
                    console.warn(`[add-phone] (${phoneTry}/${MAX_PHONE_ATTEMPTS}) 接码失败/超时(${e.message}), 换号重试`);
    ...
    async authRegisterAndAuthorizeHTTP() {
        const stepMessages = [
            "打开直接注册授权页",
            "提交注册邮箱",
        ];
    ...
    prepareManualLogin(prompt = "login") {
     ...
     */
    prepareChatGPTWebAuthorizeURL(loginHintPhone) {
        this.state = randomUrlSafeString(24);
        this.codeVerifier = randomUrlSafeString(64);
        const query = new URLSearchParams({
    ...
    async openChatGPTWebAuthorizePage(loginHintPhone) {
        ...
        if (finalURL.startsWith(`${AUTH_BASE_URL}/log-in`)) {
            ...
            throw new Error(`PHONE_ALREADY_REGISTERED: 手机号 ${loginHintPhone} 已被 OpenAI 注册（authorize 跳到 ${finalURL}），换号`);
        }
    }
    /**
     ...
     */
    async authPhoneSignupHTTP(phoneNumber, fetchPhoneCode) {
        ...
        const respReg = await this.postJSON(AUTH_REGISTER_URL, { password: this.password, username: phoneNumber }, {
            referer: `${AUTH_BASE_URL}/create-account/password`,
        ...
        const respValidate = await this.postJSON(`${AUTH_BASE_URL}/api/accounts/phone-otp/validate`, { code }, { referer: `${AUTH_BASE_URL}/contact-verification` });
        if (!respValidate.ok) {
    ...
    async authorizeContinueForSignup(screenHint = "login_or_signup") {
        const sentinelToken = await this.fetchSentinelToken("authorize_continue");
        const response = await this.postJSON(AUTH_AUTHORIZE_CONTINUE_URL, {
            username: {
                kind: "email",
                value: this.email,
            },
    ...
    async passwordVerify() {
        ...
        const response = await this.postJSON(AUTH_PASSWORD_VERIFY_URL, {
        ...
        if (!response.ok) {
            throw new Error(`PasswordVerify请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async emailOtpValidate(externalCode) {
        ...
        if (!response.ok) {
            throw new Error(`EmailOtpValidate请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async registerPassword() {
        ...
        const response = await this.postJSON(AUTH_REGISTER_URL, {
            password: this.password,
            username: this.email,
        }, {
            referer: `${AUTH_BASE_URL}/create-account/password`,
        ...
        if (!response.ok) {
            throw new Error(`RegisterPassword请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async sendEmailOtp() {
        ...
        if (!response.ok) {
            throw new Error(`EmailOtpSend请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async validatePhone(code) {
        const response = await this.postJSON(`${AUTH_BASE_URL}/api/accounts/phone-otp/validate`, { code: code }, { referer: `${AUTH_BASE_URL}/phone-verification` });
        if (!response.ok) {
            throw new Error(`PhoneOtpValidate请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async sendPhoneOtp(phoneNumber) {
        const response = await this.postJSON(`${AUTH_BASE_URL}/api/accounts/add-phone/send`, {
            phone_number: phoneNumber,
        }, {
            referer: `${AUTH_BASE_URL}/add-phone`,
        });
        if (!response.ok) {
            throw new Error(`SendPhoneOtp请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
     ...
     */
    async sendAddEmail(emailAddr) {
        const response = await this.postJSON(`${AUTH_BASE_URL}/api/accounts/add-email/send`, { email: emailAddr }, { referer: `${AUTH_BASE_URL}/add-email` });
        if (!response.ok) {
            throw new Error(`SendAddEmail请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    async selectWorkspace(consentURL) {
        ...
        if (!response.ok) {
            throw new Error(`WorkspaceSelect请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.continue_url;
    }
    ...
    async finalizeAuthorizationFromContinueURL(startURL) {
        if (startURL.startsWith(DEFAULT_REDIRECT_URI)) {
    ...
    async generateRegisterEmail() {
        if (this.email) {
            return this.email;
        }
        return getEmailAddress();
    }
    async promptEmailOtp() {
    ...
    async completeAboutYou() {
        ...
        const response = await this.postJSON(`${AUTH_BASE_URL}/api/accounts/create_account`, profile, {
            referer: `${AUTH_BASE_URL}/about-you`,
            sentinelToken,
        });
        if (!response.ok) {
            throw new Error(`CreateAccount请求失败: ${await this.formatErrorResponse(response)}`);
        }
        const payload = (await response.json());
        return payload.page?.payload?.url ?? payload.continue_url;
    }
    async finishChatGPTRegistration(callbackURL) {
    ...
    async saveChatGPTAccessToken(accessToken) {
    ...
    async resolveWorkspaceID() {
        ...
        const payload = this.decodeSignedJson(encodedPayload);
        const workspaceID = payload.workspaces?.find((w) => w.kind === "personal")?.id
    ...
    decodeSignedJson(encoded) {
        const normalized = encoded.replace(/-/g, "+").replace(/_/g, "/");
        const padded = normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "=");
        const json = Buffer.from(padded, "base64").toString("utf8");
        return JSON.parse(json);
    }
    normalizeAuthRecord(payload) {
        ...
        const accessClaims = this.decodeJwtPayload(payload.access_token);
        const idClaims = this.decodeJwtPayload(payload.id_token);
        const email = idClaims.email ?? accessClaims.email ?? this.email;
        const accountID = accessClaims["https://api.openai.com/auth"]?.chatgpt_account_id ??
    ...
    decodeJwtPayload(token) {
        ...
        return this.decodeSignedJson(parts[1]);
    }
    extractAuthResult(callbackURL) {
        ...
        if (this.state && state !== this.state) {
            throw new Error(`callback state 不匹配: expected=${this.state} actual=${state}`);
        }
        return {
            callbackURL,
            code,
    ...
    async openSignupPage(email) {
        const csrfCookie = await this.readCookie(CHATGPT_BASE_URL, "__Host-next-auth.csrf-token");
        ...
        const response = await this.fetch(`${CHATGPT_BASE_URL}/api/auth/signin/openai?${query.toString()}`, {
            method: "POST",
            redirect: "follow",
            headers: this.createBrowserHeaders({
                accept: "*/*",
    ...
    async openDirectSignupAuthorizePage(email) {
        const oauthUrl = this.prepareManualLogin();
    ...
    async fetchWithRetry(baseFetch, input, init) {
        let lastError;
        for (let attempt = 1; attempt <= FETCH_RETRY_COUNT; attempt++) {
            try {
                ...
                console.log(`[网络重试 ${attempt}/${FETCH_RETRY_COUNT}] ${this.describeRetryTarget(input)} ${this.describeRetryError(error)}`);
...
function collectErrorMessages(error) {
    ...
    if (error instanceof Error) {
        ...
        if (error.cause instanceof Error) {
            ...
            const code = "code" in error.cause ? String(error.cause.code ?? "") : "";
            if (code) {
                messages.push(code);
            }
        }
