...
export async function fetchSentinelToken(options) {
    ...
    if (useBrowserSentinel) {
        try {
            ...
            console.error(`browserSentinelTokenFailed: flow=${options.flow} error=${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
class SentinelGenerator {
    ...
    async getRequirementsToken() {
        if (!this.answers.has(this.requirementsSeed)) {
            this.answers.set(this.requirementsSeed, this.generateAnswer(this.requirementsSeed, "0"));
        }
...
async function computeTurnstileDx(requirements, key, env) {
    ...
    try {
        ...
        sdkError = error;
        console.log(`sentinelTurnstileSource: vm sdkError=${error instanceof Error ? error.message : String(error)}`);
    }
    const decoded = Buffer.from(requirements.turnstile?.dx ?? "", "base64").toString("latin1");
    ...
    try {
        ...
        throw new Error(`turnstile dx 执行失败: ops=${program.length} decodedLen=${decoded.length} sourceLen=${source.length}${sdkMessage} ${message}`);
    }
    const encoded = String(result);
    if (encoded.length <= 8) {
        throw new Error(`turnstile dx 结果异常过短: ops=${program.length} encoded=${encoded} raw=${JSON.stringify(encoded)}`);
    }
    console.log(`sentinelTurnstileSource: vm len=${encoded.length}`);
    return encoded;
}
let cachedSdkRunner = null;
...
async function loadSdkTurnstileRunner(env) {
    ...
    const patchedSource = sdkSource.replace("t.init=we,t.sessionObserverToken=async function(t){", "t.__codexTurnstileDx=function(requirements,key,dx){D(requirements,key);return _n(requirements,dx)},t.init=we,t.sessionObserverToken=async function(t){");
    ...
    const sandbox = {
        ...
        Reflect: {
            ...
            set(target, propertyKey, value, receiver) {
                ...
                const actualReceiver = (typeof receiver === "object" || typeof receiver === "function") && receiver != null
                    ? receiver
                    : target;
                try {
...
class TurnstileVM {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.drain()
                ...
                if (!this.settled) {
                    this.settled = true;
                    clearTimeout(timer);
                    reject(new Error(`turnstile vm completed without return callback: instructionCount=${this.instructionCount} queueEmpty=true recent=${JSON.stringify(this.trace.slice(-12))} slots=${JSON.stringify(this.dumpSlots([0, 1, 3, 4, 9, 10, 14, 18, 19, 28, 29, 35, 72, 79, 80, 85]))} largeSlots=${JSON.stringify(this.dumpLargeStringSlots())}`));
                }
            })
                .catch((error) => {
                if (!this.settled) {
    ...
    dumpSlots(slotIds) {
        return Object.fromEntries(slotIds.map((slotIdValue) => [
            String(slotIdValue),
    ...
    install() {
        this.state.set(0, async (payload) => {
            const nestedSource = xorCipher(Buffer.from(payload, "base64").toString("latin1"), String(this.state.get(16) ?? ""));
        ...
        this.state.set(1, (dst, src) => {
            ...
            this.state.set(dst, xorCipher(left, right));
            this.debugLog("xor", {
                dst,
                src,
                left,
        ...
        this.state.set(11, (dst, needleSlot) => {
            ...
            const script = envScripts(this.env)
        ...
        this.state.set(14, (dst, src) => {
            ...
            try {
                ...
                throw new Error(`JSON.parse failed for src ${String(src)}: ${String(error)} raw=${JSON.stringify(preview)} recent=${JSON.stringify(this.trace.slice(-12))} slots=${JSON.stringify({
        ...
        this.state.set(18, (slot) => {
            slot = slotId(slot);
            const value = Buffer.from(String(this.state.get(slot) ?? ""), "base64").toString("latin1");
            this.state.set(slot, value);
        });
        this.state.set(19, (slot) => {
            slot = slotId(slot);
            this.state.set(slot, Buffer.from(String(this.state.get(slot) ?? ""), "latin1").toString("base64"));
        });
        ...
        this.state.set(29, (dst, leftSlot, rightSlot) => {
            ...
            leftSlot = slotId(leftSlot);
            rightSlot = slotId(rightSlot);
            this.state.set(dst, Number(this.state.get(leftSlot) ?? 0) < Number(this.state.get(rightSlot) ?? 0));
        });
        this.state.set(30, (dst, resultSlot, argSlotsOrQueue, maybeQueue) => {
            ...
            const argSlots = Array.isArray(maybeQueue)
                ? argSlotsOrQueue.map((slot) => slotId(slot))
                : [];
        ...
        this.state.set(35, (dst, leftSlot, rightSlot) => {
            ...
            this.state.set(dst, divisor === 0 ? 0 : Number(this.state.get(leftSlot) ?? 0) / divisor);
        });
        ...
        this.state.set(4, (value) => {
            ...
            this.settled = true;
            this.rejectRun?.(new Error(Buffer.from(String(value), "latin1").toString("base64")));
        });
        for (const opcode of [
    ...
    async drain() {
        ...
        while (queue.length > 0 && !this.settled) {
            ...
            const handler = this.state.get(opcodeKey) ??
                this.handlers.get(opcode) ??
                this.state.get(opcode);
            if (typeof handler !== "function") {
                throw new Error(`unsupported opcode ${opcode} raw=${String(opcodeRaw)} valueType=${typeof handler} value=${String(handler)} recent=${JSON.stringify(this.trace)}`);
            }
            await handler(...args);
