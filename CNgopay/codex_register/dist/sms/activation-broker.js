...
function getActivationAgeMs(createdAt) {
    if (!createdAt) {
        return null;
    }
...
export class ActivationBroker {
    ...
    needsAnotherSms = false;
    attemptActive = false;
    round = 0;
    usage = null;
    lastReleasedUsage = null;
    activations = [];
    history = {
    ...
    getHistory() {
        return {
            ...this.history,
            phoneStats: Object.fromEntries(Object.entries(this.history.phoneStats).map(([phoneNumber, stats]) => [
                phoneNumber,
                { ...stats },
            ])),
        };
    }
    debugGetCurrentActivation() {
    ...
    async getActivation() {
        ...
        if (this.needsAnotherSms) {
            ...
            console.log(`[pollSMSCode] 复用号码[+${this.currentActivation.phoneNumber}]，第 ${(this.usage?.finishedAttemptCount ?? 0) + 1} 次， round=${this.round + 1}`);
            await this.provider.requestAnotherSms(this.currentActivation.activationId);
    ...
    async finishAttempt(outcome, rotate) {
        ...
        console.log(`[pollSMSCode] 本轮尝试完成 phone=+${activation.phoneNumber} outcome=${outcome} round=${this.round} 成功/失败=${this.getPhoneStats(activation.phoneNumber).successCount}/${this.getPhoneStats(activation.phoneNumber).failureCount}`);
    ...
    async rotateActivation(outcome) {
        const activation = this.currentActivation;
        ...
        const shouldCancelAndWithdraw = outcome === "failed" && usage.successCount === 0 &&
        ...
        console.log(`[pollSMSCode] 轮换中，对 +${activation.phoneNumber} 进行 ${action}，累计成功/失败=${usage.successCount}/${usage.failureCount}`);
    }
    async completeCurrentActivation() {
    ...
    requireCurrentActivation() {
        if (!this.currentActivation) {
            throw new Error("当前没有可用 activation");
        }
        return this.currentActivation;
    }
    isExpired(activation) {
    ...
    startAttempt() {
        ...
        if (this.currentActivation) {
            this.recordPhoneAttemptStart(this.currentActivation.phoneNumber, this.currentActivation.activationId);
            console.log(`[pollSMSCode] 进行新一轮尝试 activationId=${this.currentActivation.activationId} phone=+${this.currentActivation.phoneNumber} round=${this.round} activationAttempts=${this.usage?.startedAttemptCount ?? 0} totalAttempts=${this.history.totalAttemptsStarted}`);
        }
    }
    activate(activation) {
        if (this.usage) {
    ...
    buildLease(activation, isNewActivation, requestedAnotherSms) {
        return {
            activationId: activation.activationId,
            phoneNumber: activation.phoneNumber,
            ...
            waitForVerificationCode: async () => {
                try {
                    const verification = await this.provider.waitForVerificationCode(activation.activationId);
    ...
    recordPhoneActivation(phoneNumber, activationId) {
    ...
    recordPhoneAttemptStart(phoneNumber, activationId) {
        const stats = this.getPhoneStats(phoneNumber);
        const now = new Date();
    ...
    recordPhoneAttemptOutcome(phoneNumber, activationId, outcome) {
        const stats = this.getPhoneStats(phoneNumber);
        const now = new Date();
        if (outcome === "success") {
    ...
    recordPhoneRelease(phoneNumber, releaseMode) {
        const stats = this.getPhoneStats(phoneNumber);
        const now = new Date();
