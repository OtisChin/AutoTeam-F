    ...
    async #locateNodesByAccessibility(locator, startNodes, maxNodeCount, realm) {
        ...
        if (startNodes.length === 0) {
            ...
            for (const node of startNodes) {
                if (node.sharedId) {
                    const parsed = (0, SharedId_js_1.parseSharedId)(node.sharedId);
                    if (!parsed) {
                        throw new protocol_js_1.NoSuchNodeException(`Invalid sharedId: ${node.sharedId}`);
                    }
                    ...
                    if (node.handle) {
                        ...
                        throw new protocol_js_1.NoSuchNodeException('Start node must have sharedId or handle');
                    }
                }
            }
        }
