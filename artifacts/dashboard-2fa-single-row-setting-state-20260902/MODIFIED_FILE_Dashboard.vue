        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="(acc, i) in paginatedAccounts" :key="acc.email"
              v-memo="[acc, accountPageStartIndex, isSelected(acc.email), accountActionBusy && actionEmail === acc.email, twoFactorSubmitting || twoFactorTaskRunning, accountTwoFactorSetupInProgress(acc)]"
...
const twoFactorPendingTaskId = ref('')
...
const twoFactorTask = computed(() => {
  ...
  const task = activeTasks.find(item => {
    return item?.command === 'setup-2fa'
      && ['running', 'pending'].includes(String(item.status || ''))
  }) || props.runningTask
...
watch(twoFactorTask, task => {
  if (!twoFactorPendingTaskEmails.value.size) return
  if (task?.task_id && String(task.task_id) === twoFactorPendingTaskId.value) {
    if (twoFactorPendingClearTimer !== null) {
      clearTimeout(twoFactorPendingClearTimer)
      twoFactorPendingClearTimer = null
    }
    if (twoFactorTaskEmails.value.size) {
      twoFactorPendingTaskEmails.value = new Set(twoFactorTaskEmails.value)
    }
    return
  }
  if (!task) scheduleTwoFactorPendingTaskClear()
})
...
async function submitAccountTwoFactorSetup(accounts, scopeLabel) {
  ...
  twoFactorSubmittingEmails.value = new Set(emails.map(email => email.toLowerCase()))
  message.value = ''
  try {
    const result = await api.setupAccountsTwoFactor(emails)
    twoFactorPendingTaskId.value = String(result.task_id || '')
    twoFactorPendingTaskEmails.value = new Set(emails.map(email => email.toLowerCase()))
    if (twoFactorPendingClearTimer !== null) {
      clearTimeout(twoFactorPendingClearTimer)
      twoFactorPendingClearTimer = null
    }
