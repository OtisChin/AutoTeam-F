...
assert.match(
  dashboardSource,
  /@click="setupAccountTwoFactor\(acc\)"[\s\S]*?:disabled="twoFactorSubmitting \|\| twoFactorTaskRunning"/,
  'other per-account 2FA setup buttons should stay disabled while one setup is running',
)
assert.match(
  dashboardSource,
