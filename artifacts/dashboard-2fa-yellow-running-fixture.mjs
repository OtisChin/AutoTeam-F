...
const dist = path.resolve('D:/code/OpenSource/AutoTeam-F/src/autotoken/web/dist')
...
const server = createServer((request, response) => {
  ...
  if (url.pathname === '/api/tasks') {
    sendJson(response, [{ task_id: 'fixture-2fa', command: 'setup-2fa', status: 'running', params: { emails: ['benchmark-1@example.com'] } }])
    return
  }
  if (url.pathname === '/api/accounts/2fa/setup') {
    setTimeout(() => sendJson(response, { task_id: 'fixture-2fa' }), 1500)
    return
  }
