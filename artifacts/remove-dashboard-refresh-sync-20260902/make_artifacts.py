...
root = Path.cwd()
artifact = root / 'artifacts' / 'remove-dashboard-refresh-sync-20260902'
files = [Path('web/src/components/Dashboard.vue'), Path('web/src/api.js')]
patch_parts = []
manifest = []
for rel in files:
...
with ZipFile(artifact / 'MODIFIED_FILE', 'w', ZIP_DEFLATED) as z:
    for rel in files:
        ...
        status = 'MODIFIED' if data != (artifact / 'original' / rel).read_bytes() else 'UNCHANGED'
        manifest.append(f'{status} {digest} {rel.as_posix()}')
