...
`cloudmail.py` 当时把 `dreamhunter2333/cloudflare_temp_email` 的 `/admin/*` 路由 +
`x-admin-auth` header 硬编码成 `CloudMailClient`。命名误导，无法切换后端。

调用面：当时 `CloudMailClient` 在 8 个文件、约 **31 个调用点**被实例化或调用
...
class Email:
    ...
    id: int                       # provider 主键，统一为 int（cloudflare_temp_email 用 mails.id；cloud-mail 用 emailId）
    recipient: str                # 收件人邮箱（小写）
    sender: str                   # 发件人邮箱
    subject: str
    text: str | None              # 纯文本正文（cloudflare_temp_email 解析自 MIME；cloud-mail 取 text 字段）
    html: str | None              # HTML 正文（cloudflare_temp_email 解析自 MIME；cloud-mail 取 content/message）
    received_at: int              # epoch seconds（cloudflare_temp_email created_at 转换；cloud-mail createTime 转换）
...
class Account:
    ...
    email: str                    # 完整邮箱地址（小写）
    password: str | None = None   # cloudflare_temp_email 不返；cloud-mail 可能返
