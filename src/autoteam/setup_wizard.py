...
def check_and_setup(interactive: bool = True) -> bool:
    ...
    if not missing:
        ...
        if not _verify_temporary_email():
            if _skip:
                logger.warning("[验证] 临时邮箱服务验证失败，已根据 AUTOTEAM_SKIP_VERIFY 继续启动")
            else:
                logger.error("[验证] 临时邮箱服务配置有误，请修改 .env 后重新启动（或设置 AUTOTEAM_SKIP_VERIFY=1 跳过）")
                sys.exit(1)
        if not _verify_cpa():
            if _skip:
...
def _sniff_provider_mismatch(provider: str) -> None:
    ...
    if provider == "cloudflare_temp_email":
        base = (
            os.environ.get("CLOUDFLARE_TEMP_EMAIL_BASE_URL")
