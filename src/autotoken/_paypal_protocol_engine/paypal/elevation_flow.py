...
class IdentityElevationPayPalFlow(PayPalFlow):
    ...
    def _phase2_create_account(self):
        ...
        context_token = self.state.ec_token if str(self.state.ec_token or "").startswith("EC-") else self.ba_token
        if not context_token:
            raise RuntimeError("IDENTITY_ELEVATION_CONTEXT_TOKEN_MISSING")
        if not str(self.state.ec_token or "").startswith("EC-"):
            logger.warning("Identity-elevation onboarding produced no EC token; continuing with BA checkout context")
        ...
        if not self.state.signup_url:
            self.state.signup_url = "https://www.paypal.com/checkoutweb/signup"
        if not self.state.content_identifier:
            self.state.content_identifier = f"{self.address.country}:{self.lang}:compliance.signupTerms"
    ...
    def _phase3_signup_and_2fa(self):
        context_token = self.state.ec_token if str(self.state.ec_token or "").startswith("EC-") else self.ba_token
        if not context_token:
            raise RuntimeError("IDENTITY_ELEVATION_CONTEXT_TOKEN_MISSING: signup requires a checkout token")
    ...
    def _protocol_identity_elevation(self) -> dict:
        token = self.state.ec_token if str(self.state.ec_token or "").startswith("EC-") else self.ba_token
        if not token:
            raise RuntimeError("IDENTITY_ELEVATION_CONTEXT_TOKEN_MISSING")
