...
def _env_truthy(name: str, default: bool = False) -> bool:
    raw = (_load_dotenv_value(name) or "").strip().lower()
    if not raw:
        return default
    return raw in {"1", "true", "yes", "on", "y", "enable", "enabled"}


def _load_headless_pinned_runtime_profile() -> JsonDict | None:
    """Load a previously captured runtime fingerprint for headless mode.

    This lets local headless reuse the same browser_profile/screen/viewport
    captured from Roxy (for example the current iOS/iPhone profile) instead of
    re-capturing a desktop Chromium profile.
    """
    if not _env_truthy("PAYPAL_HEADLESS_USE_PINNED_FINGERPRINT", False):
        return None
    from pathlib import Path

    raw_path = (
        _load_dotenv_value("PAYPAL_HEADLESS_PINNED_FINGERPRINT_PATH")
        or _load_dotenv_value("PAYPAL_PINNED_FINGERPRINT_PATH")
        or "var/roxy_ios_fingerprint_current.json"
    )
    ...
    if not all(isinstance(item, dict) for item in (profile, screen, viewport, fp)):
        return None
    runtime: JsonDict = {
        "browser_profile": dict(cast(JsonDict, profile)),
        "screen": dict(cast(JsonDict, screen)),
        "viewport": dict(cast(JsonDict, viewport)),
        "device_fingerprint": dict(cast(JsonDict, fp)),
    }
    runtime["browser_profile"]["fingerprint_source"] = "headless"
    runtime["browser_profile"]["headless_pinned_fingerprint_path"] = str(path)
    runtime["device_fingerprint"]["source"] = "headless"
    runtime["device_fingerprint"]["pinned_source"] = str(fp.get("source") or "")
    return runtime
    ...
    *,
    ...
    if selected == "headless":
        try:
            ...
            pinned_runtime = _load_headless_pinned_runtime_profile()
            if pinned_runtime:
                browser_profile = cast(dict[str, object], pinned_runtime["browser_profile"])
                screen = cast(dict[str, object], pinned_runtime["screen"])
                viewport = cast(dict[str, object], pinned_runtime["viewport"])
                logger.info(
                    "Local headless using pinned fingerprint: ua={} screen={}x{} viewport={}x{}",
