...
def _chrome_user_agent_metadata(profile: JsonObject) -> JsonObject:
    ...
    return {
        ...
        "model": str(profile.get("sec_ch_ua_model") or "").strip('"'),
        "mobile": bool(profile.get("is_mobile", False)),
...
def _headless_extra_http_headers(profile: JsonObject) -> dict[str, str]:
    ...
    return {
        ...
        "Sec-CH-UA-Model": str(profile.get("sec_ch_ua_model") or '""'),
...
def _stealth_init_script(
    *,
    ...
    config = {
        ...
        "uaData": ua_data,
        "omitUserAgentData": bool(profile.get("omit_client_hints", False)),
    }
    return f"""
(() => {{
  ...
  const patchNavigatorLike = (proto) => {{
    ...
    defineGetter(proto, "maxTouchPoints", () => cfg.maxTouchPoints || 0);
    defineGetter(proto, "userAgentData", () => cfg.omitUserAgentData ? undefined : makeUAData());
  }};
    ...
    if stage == "signup_context":
        rules.extend(
            [
                ...
                HeadlessAllowlistRule("www.paypalobjects.com", "/webcaptcha/ngrlCaptcha.min.js", methods=("GET",), resource_types=("script",), reason="signup_context_webcaptcha_script"),
