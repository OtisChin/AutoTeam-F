...
IOS_SCREEN = {"width": 390, "height": 844, "availWidth": 390, "availHeight": 844, "colorDepth": 24, "pixelDepth": 24}
IOS_VIEWPORT = {"width": 390, "height": 753}


def _roxy_ios_profile_enabled() -> bool:
    raw = (
        _env_str("PAYPAL_ROXY_DEVICE_PROFILE")
        or _env_str("PAYPAL_DEVICE_PROFILE")
        or ("ios" if _env_bool("PAYPAL_ROXY_FORCE_IOS", False) else "")
    ).strip().lower().replace("-", "_")
    return raw in {"ios", "iphone", "iphone_ios", "mobile_ios", "safari_ios"}


def _ios_user_agent() -> str:
    return _env_str("PAYPAL_ROXY_IOS_UA") or _env_str("PAYPAL_IOS_UA") or IOS_USER_AGENT


def _ios_int(name: str, default: int) -> int:
    raw = _env_str(name)
    if raw:
        try:
            return int(raw)
...
def _ios_screen() -> dict[str, int]:
    width = _ios_int("PAYPAL_ROXY_IOS_SCREEN_WIDTH", int(IOS_SCREEN["width"]))
    height = _ios_int("PAYPAL_ROXY_IOS_SCREEN_HEIGHT", int(IOS_SCREEN["height"]))
    return {
        "width": width,
        "height": height,
        "availWidth": width,
...
def _ios_viewport() -> dict[str, int]:
    return {
        "width": _ios_int("PAYPAL_ROXY_IOS_VIEWPORT_WIDTH", int(IOS_VIEWPORT["width"])),
        "height": _ios_int("PAYPAL_ROXY_IOS_VIEWPORT_HEIGHT", int(IOS_VIEWPORT["height"])),
    }


def _ios_profile_overrides() -> dict[str, Any]:
    screen = _ios_screen()
    viewport = _ios_viewport()
    return {
        "fingerprint_source": "roxy_ios",
        "device_profile": "ios",
        "user_agent": _ios_user_agent(),
        "country": "BR",
        "language": "pt-BR",
        "locale": "pt_BR",
        ...
        "sec_ch_platform": '"iOS"',
        "sec_ch_platform_version": '"17.6"',
        "sec_ch_arch": '"arm"',
        "sec_ch_bitness": "",
        "sec_ch_ua_mobile": "?1",
        "sec_ch_ua_model": '"iPhone"',
        "omit_client_hints": True,
        "is_mobile": True,
        "has_touch": True,
        "max_touch_points": 5,
        ...
        "inner_width": viewport["width"],
        "inner_height": viewport["height"],
        "outer_width": screen["width"],
        "outer_height": screen["height"],
        "browser_markers": ["safari", "mobile", "webkit"],
    }
...
class RoxyApiClient:
    ...
    def create_profile(self, workspace_id: int, project_id: int | None) -> str:
        ...
        ios_profile = _roxy_ios_profile_enabled()
        open_width = _ios_viewport()["width"] if ios_profile else self.config.open_width
        open_height = _ios_viewport()["height"] if ios_profile else self.config.open_height
        screen_width = _ios_screen()["width"] if ios_profile else self.config.screen_width
        screen_height = _ios_screen()["height"] if ios_profile else self.config.screen_height
        payload: dict[str, Any] = {
            "workspaceId": workspace_id,
            "windowName": f"paypal-fp-{uuid.uuid4().hex[:10]}",
            "coreType": "Chrome",
            # Roxy Local API currently rejects "iOS" in the `os` field for
            # Chrome profiles; keep a supported desktop OS and apply iPhone
            # UA/touch/viewport overrides through CDP after opening.
            "os": self.config.os_name,
            "osVersion": self.config.os_version,
            "cookie": [],
            "searchEngine": "Google",
            "defaultOpenUrl": ["about:blank"],
            "windowRemark": "paypal runtime iOS fingerprint capture" if ios_profile else "paypal runtime fingerprint capture",
            ...
            "fingerInfo": {
                ...
                "forbidImage": False,
                "forbidMedia": False,
                "openWidth": str(open_width),
                "openHeight": str(open_height),
                "openBookmarks": False,
                ...
                "openWorkbench": 0,
                "resolutionType": True,
                "resolutionX": str(screen_width),
                "resolutionY": str(screen_height),
                "fontType": True,
                "webRTC": 2,
                "webGL": True,
                "webGLInfo": True,
...
"""
    with sync_playwright() as p:
        ...
        if _roxy_ios_profile_enabled():
            ios_screen = _ios_screen()
            ios_viewport = _ios_viewport()
            try:
                page.set_viewport_size({"width": ios_viewport["width"], "height": ios_viewport["height"]})
            except Exception:
                pass
            try:
                cdp = context.new_cdp_session(page)
                cdp.send("Emulation.setUserAgentOverride", {
                    "userAgent": _ios_user_agent(),
                ...
                cdp.send("Emulation.setDeviceMetricsOverride", {
                    ...
                    "deviceScaleFactor": 3,
                    "mobile": True,
                    "screenWidth": ios_screen["width"],
                    "screenHeight": ios_screen["height"],
                    "positionX": 0,
                    "positionY": 0,
                })
                cdp.send("Emulation.setTouchEmulationEnabled", {"enabled": True, "maxTouchPoints": 5})
            except Exception as exc:
                logger.debug("Roxy iOS CDP emulation setup skipped/failed: {}", exc)
            page.add_init_script(
                ...
                (() => {
                  ...
                  define(Navigator.prototype, 'platform', 'iPhone');
...
def _runtime_to_profile(js: dict[str, Any], cdp_info: dict[str, Any]) -> dict[str, Any]:
    ...
    if _roxy_ios_profile_enabled():
        profile.update(_ios_profile_overrides())
    return profile
...
def _runtime_device_fingerprint(js: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "source": "roxy_ios" if _roxy_ios_profile_enabled() else "roxy",
