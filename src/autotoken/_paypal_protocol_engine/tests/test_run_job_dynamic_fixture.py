...
def test_run_job_keeps_paypal_success_when_braintree_context_bridge_is_down():
    proxy = ProxyConfig(True, ProxyEntry("127.0.0.1", 9999, "", ""))
    job = web.WebJob(
        id="paypalonlyjob",
        owner_device_id="fixture-device",
    ...
    with patch("web.select_working_proxy", return_value=proxy), patch(
        "web.generate_card", return_value=CardInfo("4111111111111111", "12/2030", "123")
    ), patch("web.WebIdentityElevationPayPalFlow", FakeFlow), patch("web.httpx.Client", RefusedContextClient):
        web.run_job(job)
    assert job.status == "completed"
    assert job.result["status"] == "success"
    assert "BRAINTREE_VAULT_FAILED" not in str(job.result)


if __name__ == "__main__":
    test_run_job_dynamic_country_fixture()
    test_run_job_keeps_paypal_success_when_braintree_context_bridge_is_down()
