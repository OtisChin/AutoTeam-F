...
PAYPAL_PROTOCOL_INTERNAL_BASE = str(
    os.getenv(
        "PAYPAL_PROTOCOL_INTERNAL_BASE_URL",
        os.getenv("PAYPAL_WEB_PAY153_INTERNAL_BASE", "http://127.0.0.1:8799"),
    )
).rstrip("/")
                ...
                try:
                    with httpx.Client(timeout=httpx.Timeout(180.0), trust_env=False) as client:
                        context_response = client.post(
                            f"{PAYPAL_PROTOCOL_INTERNAL_BASE}/api/grok-trial/braintree-context",
                            json={"billing_token": job.ba_token},
                        )
                        context_data = context_response.json() if context_response.status_code == 200 else {}
                        raw_context = context_data.get("result") if isinstance(context_data, dict) else {}
                        context = raw_context if isinstance(raw_context, dict) else {}
                except Exception as context_error:
                    logger.warning(
                        "Braintree context bridge unavailable; continuing with PayPal-only success: {}",
                        redact_text(context_error),
                    )
                    context = {}
                    ...
                    try:
                        with httpx.Client(timeout=httpx.Timeout(180.0), trust_env=False) as client:
                            ...
                            complete_response = client.post(
                                f"{PAYPAL_PROTOCOL_INTERNAL_BASE}/api/grok-trial/braintree-complete",
                                json={
