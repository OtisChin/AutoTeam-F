"""Philippines GCash link extraction routes."""
...
from autotoken.payments.gcash_ph import GCashPhJobConfig, detect_gcash_eligibility, generate_gcash_ph_trial
...
LINKS_FILE = PROJECT_ROOT / "data" / "gcash_ph_links.json"
ACCOUNT_STATUS_FILE = PROJECT_ROOT / "data" / "gcash_ph_account_status.json"
GCASH_LINK_TTL_SECONDS = 10 * 60
MAX_BATCH_CONCURRENCY = 20
MAX_ACCOUNT_ATTEMPTS = 5
MAX_CONFIGURABLE_ACCOUNT_ATTEMPTS = 20
GCASH_STATUS_PENDING = "pending"
GCASH_STATUS_ELIGIBLE = "eligible"
GCASH_STATUS_INELIGIBLE = "ineligible"
GCASH_STATUS_RUNNING = "running"
GCASH_STATUS_FAILED = "failed"
GCASH_STATUS_SUCCESS = "success"
GCASH_STATUS_PAID = "paid"
GCASH_STATUS_TEXT = {
    GCASH_STATUS_PENDING: "未提链",
    GCASH_STATUS_ELIGIBLE: "有资格",
    GCASH_STATUS_INELIGIBLE: "无资格",
    GCASH_STATUS_RUNNING: "提链中",
    GCASH_STATUS_FAILED: "提链失败",
    GCASH_STATUS_SUCCESS: "已提链",
    GCASH_STATUS_PAID: "已支付",
}
ACCOUNT_UI_FIELDS = (
...
class GCashPhStartRequest(BaseModel):
...
class GCashPhBatchStartRequest(GCashPhStartRequest):
...
class GCashPhDeleteLinksRequest(BaseModel):
    ids: list[str] = Field(default_factory=list)


class GCashPhDeleteAccountsRequest(BaseModel):
...
def _set_account_status(email: str, status: str, *, error: str = "", job_id: str = "") -> dict[str, Any]:
    ...
    if not key:
        return {}
    normalized = str(status or GCASH_STATUS_PENDING).strip().lower()
    if normalized not in GCASH_STATUS_TEXT:
        normalized = GCASH_STATUS_PENDING
...
def _gcash_paid_emails() -> set[str]:
    paid: set[str] = set()
    try:
    ...
    for account in accounts:
        ...
        gcash_bound = bind_provider == "gcash_ph" and bind_status in {"success", "succeeded", "ok"}
        if account_type == account_store.ACCOUNT_TYPE_PLUS or status == account_store.STATUS_PLUS or gcash_bound:
            paid.add(email)
    return paid
...
def _batch_concurrency(req: GCashPhBatchStartRequest, total: int) -> int:
    try:
...
def _account_attempt_limit(req: GCashPhBatchStartRequest) -> int:
    try:
...
def _dedupe_link_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        link = _normalize_link(item.get("gcash_link") or item.get("provider_redirect_url") or item.get("stripe_redirect_url"))
        if email:
...
def _load_links_pruning_deleted_accounts() -> tuple[list[dict[str, Any]], int]:
    ...
    paid_emails = _gcash_paid_emails()
...
def _iter_auth_accounts_with_gcash_status() -> list[dict[str, Any]]:
    statuses = _load_account_statuses()
    paid_emails = _gcash_paid_emails()
    ...
    for account in _iter_auth_accounts(include_paid=True):
        ...
        if key in paid_emails:
            item = {"status": GCASH_STATUS_PAID, "error": "", "updated_at": ""}
        elif not item and key in linked_emails:
            item = {"status": GCASH_STATUS_SUCCESS, "error": "", "updated_at": ""}
        status = str(item.get("status") or GCASH_STATUS_PENDING)
        if status not in GCASH_STATUS_TEXT:
            status = GCASH_STATUS_PENDING
        rows.append({
            field: (email if field == "email" else dashboard_account.get(field, account.get(field))) for field in ACCOUNT_UI_FIELDS
        } | {
            "gcash_status": status,
            "gcash_status_text": GCASH_STATUS_TEXT[status],
            "gcash_error": str(item.get("error") or ""),
            "gcash_country": "PH",
            "gcash_status_updated_at": item.get("updated_at"),
            "gcash_selectable": status != GCASH_STATUS_PAID,
        })
    return rows


def _select_batch_accounts(req: GCashPhBatchStartRequest) -> list[dict[str, Any]]:
...
def _link_record_from_result(job_id: str, account_email: str, result: dict[str, Any]) -> dict[str, Any]:
    ...
    primary_link = str(fields.get("provider_redirect_url") or fields.get("gcash_link") or fields.get("stripe_redirect_url") or "")
    currency = str(fields.get("currency") or result.get("currency") or "PHP").strip().upper() or "PHP"
    created_at_ts = int(time.time())
    explicit_expires_at_ts = _timestamp_seconds(fields.get("gcash_expires_at_ts") or fields.get("gcash_expires_at") or result.get("gcash_expires_at_ts") or result.get("gcash_expires_at"))
    expires_at_ts = explicit_expires_at_ts or (created_at_ts + GCASH_LINK_TTL_SECONDS)
    return {
        ...
        "gcash_qr_url": str(fields.get("gcash_qr_url") or ""),
        "gcash_qr_data": str(fields.get("gcash_qr_data") or ""),
        "gcash_qr_image": str(fields.get("gcash_qr_image") or ""),
        "gcash_ttl_seconds": max(0, expires_at_ts - created_at_ts),
        "gcash_expires_at_ts": expires_at_ts,
        "billing": billing,
    }
...
def _should_retry_gcash_error(message: str) -> bool:
    ...
    non_retry_markers = (
        ...
        "oaics checkout cannot use stripe payment_pages gcash flow",
    )
...
def _run_batch_account(
    ...
    if _is_job_cancel_requested(job_id):
        ...
        return {"ok": False, "email": email, "skipped": True, "reason": "任务已取消", "status": _set_account_status(email, GCASH_STATUS_PENDING, job_id=job_id)}

    if str(email).lower() in _gcash_paid_emails():
        status = _set_account_status(email, GCASH_STATUS_PAID, job_id=job_id)
        return {"ok": False, "email": email, "skipped": True, "reason": "账号已支付", "status": status}

    attempts = 0
    _set_job_running_delta(job_id, 1)
    try:
        _set_account_status(email, GCASH_STATUS_RUNNING, job_id=job_id)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                eligibility = detect_gcash_eligibility(cfg, account_log)
                if str(eligibility.get("status") or "").lower() != GCASH_STATUS_ELIGIBLE or not eligibility.get("has_gcash"):
                    status = _set_account_status(email, GCASH_STATUS_INELIGIBLE, job_id=job_id)
                    account_log("资格检测结果：无 GCash")
                    return {"ok": False, "email": email, "skipped": True, "reason": "无资格", "status": status}
                status = _set_account_status(email, GCASH_STATUS_ELIGIBLE, job_id=job_id)
                if req.qualification_only:
                    account_log("资格检测结果：有 GCash")
                ...
                result = generate_gcash_ph_trial(replace(cfg, preflight_result=eligibility), account_log)
                break
                ...
                if pix_routes._is_token_invalidated_error(last_error) or pix_routes._is_no_organization_error(last_error):
                    ...
                    status = _set_account_status(email, GCASH_STATUS_FAILED, error=last_error, job_id=job_id)
                ...
                if pix_routes._is_non_zero_after_promo_error(last_error):
                    status = _set_account_status(email, GCASH_STATUS_FAILED, error=last_error, job_id=job_id)
                    _append_log(job_id, f"[{index}/{total}] GCash PH 金额非 0，账号保留：{email}")
                    return {
                        "ok": False,
                        "email": email,
                        "error": {
                            ...
                            "elapsed_s": round(time.monotonic() - started, 1),
                            "attempts": attempt,
                            "error": f"GCash PH 金额非 0，账号保留：{last_error}",
                ...
                if attempt >= max_attempts or not _should_retry_gcash_error(last_error):
                    raise
        ...
        _append_link(record)
        status = _set_account_status(email, GCASH_STATUS_SUCCESS, job_id=job_id)
        ...
        error = str(exc)
        status = _set_account_status(email, GCASH_STATUS_FAILED, error=error, job_id=job_id)
...
def _run_batch_job(job_id: str, req: GCashPhBatchStartRequest) -> None:
    ...
    try:
        ...
        for account in accounts:
            email = str(account.get("email") or "").strip()
            account_statuses[email] = _set_account_status(email, GCASH_STATUS_PENDING, job_id=job_id)
        with JOBS_LOCK:
        ...
        log(f"GCash PH 任务开始：{len(accounts)} 个账号，并发 {concurrency}，模式={mode_text}")
        completed = 0
        ...
        log(f"GCash PH 任务完成：成功 {len(successes)}，失败 {len(errors)}，跳过 {len(skipped)}")
    except Exception as exc:
        with JOBS_LOCK:
...
def _delete_gcash_ph_account_everywhere(email: str) -> dict[str, Any]:
    clean_email = str(email or "").strip()
    gcash_cleanup = delete_account_artifacts(clean_email)
    ...
    return {"ok": True, "email": clean_email, "dashboard_account_deleted": dashboard_account_deleted, "auth_session_deleted": auth_session_deleted, "gcash_ph": gcash_cleanup}


def create_gcash_ph_router() -> APIRouter:
    router = APIRouter()

    @router.get("/api/gcash-ph/accounts")
    def get_gcash_ph_accounts() -> dict[str, Any]:
        return {"accounts": _iter_auth_accounts_with_gcash_status()}

    @router.delete("/api/gcash-ph/accounts/{email}")
    def delete_gcash_ph_account(email: str) -> dict[str, Any]:
        clean_email = str(email or "").strip()
        if not clean_email:
            raise HTTPException(status_code=400, detail="email required")
        return _delete_gcash_ph_account_everywhere(clean_email)

    @router.post("/api/gcash-ph/accounts/delete")
    def delete_gcash_ph_accounts(req: GCashPhDeleteAccountsRequest) -> dict[str, Any]:
        ...
        return {"ok": True, "results": [_delete_gcash_ph_account_everywhere(email) for email in emails]}

    @router.post("/api/gcash-ph/start")
    def start_gcash_ph(req: GCashPhStartRequest) -> dict[str, str]:
        emails = [str(req.account_email or "").strip()] if str(req.account_email or "").strip() else []
        batch_req = GCashPhBatchStartRequest.model_validate(req.model_dump(by_alias=True) | {"accountEmails": emails})
        job_id = _new_job(emails, 1)
        threading.Thread(target=_run_batch_job, args=(job_id, batch_req), daemon=True).start()
        return {"job_id": job_id}

    @router.post("/api/gcash-ph/batch/start")
    def start_gcash_ph_batch(req: GCashPhBatchStartRequest) -> dict[str, str]:
    ...
    @router.post("/api/gcash-ph/jobs/{job_id}/cancel")
    def cancel_gcash_ph_job(job_id: str) -> dict[str, Any]:
        with JOBS_LOCK:
            job = JOBS.get(job_id)
            if not job:
                raise HTTPException(status_code=404, detail="job not found")
    ...
    @router.get("/api/gcash-ph/jobs/{job_id}")
    def get_gcash_ph_job(job_id: str) -> dict[str, Any]:
        return _job_snapshot(job_id)

    @router.get("/api/gcash-ph/links")
    def get_gcash_ph_links() -> dict[str, Any]:
    ...
    @router.post("/api/gcash-ph/links/delete")
    def delete_gcash_ph_links(req: GCashPhDeleteLinksRequest) -> dict[str, Any]:
    ...
    @router.post("/api/gcash-ph/links/clear")
    def clear_gcash_ph_links() -> dict[str, Any]:
