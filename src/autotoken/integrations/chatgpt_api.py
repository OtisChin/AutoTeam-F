...
from autotoken.core.browser_fingerprint import (
...
from autotoken.integrations.proxy_bridge import start_playwright_socks_bridge
...
def _camoufox_effective_proxy_url(proxy_url: str | None):
    ...
    try:
        ...
        parsed = None
    if parsed and parsed.scheme in {"socks5", "socks5h"} and not parsed.username and not parsed.password:
...
class ChatGPTTeamAPI:
    ...
    def _clear_browser_runtime_data(self, *, reason: str, browser_label: str, on_progress=None) -> None:
        ...
        if cleared:
            logger.info(
                "[ChatGPT] browser runtime data cleared before account flow: %s reason=%s", ",".join(cleared), reason
            )
        _emit_browser_progress(
            on_progress,
    ...
    def _launch_browser_roxybrowser(
        ...
        *,
        ...
        window_name = f"autotoken-chatgpt-{uuid.uuid4().hex[:8]}"
    ...
    def _extract_session_token(self):
        session_token = chatgpt_session_service.session_token_from_cookie_items(self.context.cookies())
    ...
    def _list_real_workspaces(self):
        ...
        try:
            ...
            raise RuntimeError(f"/backend-api/accounts 响应解析失败: {exc}") from exc
