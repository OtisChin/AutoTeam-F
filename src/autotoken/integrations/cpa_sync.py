...
from autotoken.settings.config import CPA_KEY, CPA_URL
...
def _auth_dir_codex_files():
    yield from iter_codex_auth_files(auth_dir=AUTH_DIR)


def _auth_files_for_email(email: str):
    yield from iter_auth_files_for_email(email, auth_dir=AUTH_DIR)


def _delete_local_auth_file(path: Path) -> bool:
    if not is_inside_auth_dir(path, auth_dir=AUTH_DIR):
        logger.warning("[CPA] 跳过认证目录外的文件删除: %s", path)
        return False
    return delete_auth_file(path, auth_dir=AUTH_DIR)
...
def ensure_cpa_compatible_auth_file(filepath, *, fallback_email: str = "", fallback_plan_type: str = "") -> str:
    ...
    if not path.exists():
        return ""
    try:
        auth_data = read_auth_json_file(path)
    except Exception:
...
def find_cpa_compatible_auth_file(email: str, preferred_path: str = "", *, fallback_plan_type: str = "") -> str:
    ...
    if AUTH_DIR.exists():
        candidates.extend(_auth_dir_codex_files())
    ...
    for candidate in candidates:
        ...
        if not prepared:
            continue
        try:
            auth_data = read_auth_json_file(prepared)
        except Exception:
            continue
        bundle = _bundle_from_auth_data(auth_data, fallback_name=Path(prepared).name)
        if target_email and _normalized_email(bundle.get("email")) != target_email:
            continue
        return prepared
    return ""
...
def _auth_identity(bundle, main=False):
    ...
    return ("codex", _normalized_email(bundle.get("email")), bundle.get("account_id") or "")
...
def _write_auth_file(filepath, bundle):
    ensure_auth_dir()
    filepath = Path(filepath)
    if not is_inside_auth_dir(filepath, auth_dir=AUTH_DIR):
        raise ValueError(f"Refusing to write auth file outside AUTH_DIR: {filepath}")
    auth_data = {
        "type": "codex",
        "disabled": False,
    ...
    *,
    ...
    for candidate in candidates:
        ...
        if not isinstance(auth_data, dict):
            continue
        bundle = _bundle_from_auth_data(auth_data, fallback_name=path.name)
        bundle_email = _normalized_email(bundle.get("email"))
...
def _save_normalized_auth_file(bundle, main=False):
    ...
    if main:
        for old in _auth_dir_codex_files():
            if not old.name.startswith("codex-main-"):
                continue
            if old != filepath and old.exists():
                delete_codex_auth_file(old)
                _delete_local_auth_file(old)
    else:
        email = bundle.get("email", "")
        for old in _auth_files_for_email(email):
            if old != filepath and old.exists():
                delete_codex_auth_file(old)
                _delete_local_auth_file(old)
...
def _load_local_best_candidate(identity_key):
    ...
    best = None
    for path in _auth_dir_codex_files():
        if not path.is_file():
            continue
        try:
            auth_data = read_auth_json_file(path)
    ...
    for items in grouped.values():
        ...
        for item in items:
            if item["path"] != canonical_path and item["path"].exists():
                if _delete_local_auth_file(item["path"]):
                    removed += 1
...
def sync_from_cpa():
    ...
    """
    from autotoken.storage.accounts import STATUS_STANDBY, find_account, load_accounts, save_accounts
    ...
    for item in cpa_files:
        ...
        email = _normalized_email(bundle.get("email") or item.get("email"))
        bundle["email"] = email
        ...
        if winner["main"]:
            ...
            if normalized_path.name != name:
                ...
                if old_path.exists() and old_path != normalized_path:
                    _delete_local_auth_file(old_path)
            continue
...
def sync_to_cpa():
    ...
    """
    from autotoken.storage.accounts import STATUS_ACTIVE, STATUS_PERSONAL, STATUS_PLUS, load_accounts, save_accounts

    accounts = load_accounts()
    local_emails = {_normalized_email(a.get("email")) for a in accounts if _normalized_email(a.get("email"))}
    local_duplicates_deleted, accounts_path_repaired = _cleanup_local_duplicates(accounts)
    if accounts_path_repaired:
        save_accounts(accounts)
    ...
    for acc in accounts:
        ...
        if auth_path and not Path(auth_path).exists():
            matches = list(_auth_files_for_email(acc.get("email", "")))
            if matches:
