...
from autotoken.core.jwt import decode_jwt_payload as _core_decode_jwt_payload
...
class ConversionError(ValueError):
    """Raised when a CPA source or Sub2API export setting is invalid."""
...
def load_source_record_from_text(file_name: str, content: str) -> SourceRecord:
    try:
        if not file_name.lower().endswith(".json"):
            raise ConversionError("只支持 JSON 文件。")
        raw_data = json.loads(content.lstrip("\ufeff"))
...
def refresh_target_names(records: Sequence[SourceRecord]) -> None:
    ...
    for record in ordered:
        base_name = derive_name_from_email(record.email) or Path(record.file_name).stem
...
def inspect_sources(files: Sequence[tuple[str, str]]) -> list[SourceRecord]:
    records = [load_source_record_from_text(file_name, content) for file_name, content in files]
...
def export_records(
    ...
    for record in records:
        record.selected = record.is_valid and (
            selected_file_names is None or record.file_name in selected_file_names
        )
    refresh_target_names(records)
...
def decode_jwt_payload(token: str) -> dict[str, Any]:
    return _core_decode_jwt_payload(token)
