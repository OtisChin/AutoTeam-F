...
用法:
    autotoken check     # 检查所有活跃账号额度
    autotoken rotate    # 执行一次轮转（检查 + 替换）
    autotoken add       # 手动添加一个新账号
    autotoken status    # 查看所有账号状态
"""
...
def _valid_account_auth_path(acc: dict) -> Path | None:
    auth_file = _valid_account_auth_file(acc)
    return Path(auth_file) if auth_file else None
...
def _get_account_login_provider(acc: dict | None) -> str:
    return rotation.account_login_provider(acc)


def _auto_reuse_skip_reason(acc: dict | None) -> str | None:
    return rotation.auto_reuse_skip_reason(acc)
...
def _find_team_auth_file(email):
    ...
    """
    try:
        from autotoken.storage.auth_files import iter_auth_files_for_email
        from autotoken.storage.auth_storage import AUTH_DIR
    except Exception:
        return None
    if not AUTH_DIR.exists():
        return None
    candidates = sorted(iter_auth_files_for_email(email, auth_dir=AUTH_DIR, plan_type="team"))
    return str(candidates[0]) if candidates else None


def _is_quota_exhausted_snapshot(acc):
    """本地 last_quota 表明 5h 和周额度均满(pct=100)→ 耗尽未抛弃。"""
    return reconcile_service.quota_exhausted_snapshot(acc)
...
def _reconcile_team_members(chatgpt_api=None, *, dry_run=False):
    ...
    """
    from autotoken.settings.config import RECONCILE_KICK_GHOST, RECONCILE_KICK_ORPHAN
    ...
    try:
        ...
        if excess > 0:
            ...
            def _priority(email):
                return reconcile_service.over_cap_priority(email, acc_map, kick_ghost=RECONCILE_KICK_GHOST)

            victims = reconcile_service.over_cap_victims(
                remaining_subs,
                acc_map,
                excess=excess,
                kick_ghost=RECONCILE_KICK_GHOST,
            )
            if dry_run:
...
def sync_account_states(chatgpt_api=None):
    ...
    register_domain = CLOUD_MAIL_DOMAIN or CLOUDFLARE_TEMP_EMAIL_DOMAIN or ""
    domain_suffix = register_domain.lstrip("@") if register_domain else ""
    ...
    from autotoken.auth.codex_auth import AUTH_DIR
...
def _status_table_symbols(stream=None) -> dict[str, str]:
...
def _print_status_table(accounts, quota_cache=None):
    ...
    symbols = _status_table_symbols()
    STATUS_STYLE = {
        STATUS_ACTIVE: ("bold green", f"{symbols['active']} active"),
        STATUS_EXHAUSTED: ("bold red", f"{symbols['exhausted']} used up"),
        STATUS_STANDBY: ("yellow", f"{symbols['standby']} standby"),
        STATUS_PENDING: ("dim", f"{symbols['pending']} pending"),
    }
    ...
    console.print(
        f"  [green]{symbols['active']} 活跃 {active}[/]  "
        f"[yellow]{symbols['standby']} 待命 {standby}[/]  "
        f"[red]{symbols['exhausted']} 用完 {exhausted}[/]  "
...
def cmd_status():
    ...
    # active 账号实时查询额度
    quota_cache = {}
    active_count = sum(1 for a in accounts if a["status"] == STATUS_ACTIVE and _valid_account_auth_path(a))
    if active_count:
        logger.info("[状态] 查询 %d 个 active 账号额度...", active_count)
    for acc in accounts:
        auth_path = _valid_account_auth_path(acc)
        if acc["status"] == STATUS_ACTIVE and auth_path:
            try:
                auth_data = read_auth_json_file(auth_path)
            except Exception as exc:
                logger.warning("[状态] 跳过无效 auth 文件 %s: %s", acc.get("email"), exc)
                continue
...
def _check_and_refresh(acc):
    ...
    try:
        ...
        logger.warning("[%s] auth 文件无效: %s", email, exc)
        return "no_auth", None
    access_token = auth_data.get("access_token")
    ...
    if status == "auth_error" and rt:
        ...
        if new_tokens:
            ...
            try:
                ...
                upsert_codex_auth_file(auth_path, auth_data, main=auth_path.name.startswith("codex-main-"))
            except Exception:
                pass
...
def cmd_check(include_standby: bool = False):
    ...
    # API 运行时配置优先（前端可修改）
    try:
        from autotoken.interfaces.api import _auto_check_config
    ...
    def _check_personal_accounts(threshold):
        ...
        """
        from autotoken.storage.accounts import load_accounts as _reload

        personal_accs = [
            a for a in _reload() if a["status"] == STATUS_PERSONAL and not _is_main_account_email(a.get("email"))
        ]
        personal_with_auth = [a for a in personal_accs if _valid_account_auth_path(a)]
    ...
    no_auth_list = []
    for a in all_active:
        if _valid_account_auth_path(a):
            active_with_auth.append(a)
        else:
            # 只管我们域名的账号
            register_domain = (CLOUD_MAIL_DOMAIN or CLOUDFLARE_TEMP_EMAIL_DOMAIN or "").lstrip("@")
            if register_domain and register_domain in a["email"]:
...
def _probe_standby_quota():
    ...
    no_auth = 0
    for acc in standby:
        if not _valid_account_auth_path(acc):
            no_auth += 1
            continue
...
def _run_post_register_oauth(
    ...
    update_account(email, **registration.team_auth_missing_update_fields())
    logger.warning("[注册] 账号已加入 Team 但 Codex 登录失败,需要补登录: %s", email)
    _record_outcome("team_auth_missing", reason=registration.TEAM_AUTH_MISSING_REASON)
    return email
...
def _auth_session_payload_has_web_session(session_data) -> bool:
    ...
    return bool(session_token or cookie_header)


def _refresh_auth_session_via_protocol_login(
    email,
    password,
    mail_client,
    *,
    ...
    session_data = protocol_login_once(
        mail_client,
        email=email,
        password=password,
...
def _save_codex_oauth_bundle_for_account(
    ...
    *,
    ...
    bundle = registration.free_codex_oauth_bundle(bundle, email=email)
    plan_type = bundle["plan_type"]
    seat_type = registration.team_success_seat_label(plan_type)
    auth_file = save_auth_file(bundle)
...
def _run_post_register_session_oauth(
    ...
    *,
    ...
    bundle = registration.free_codex_oauth_bundle(oauth_result["bundle"])
    plan_type = bundle["plan_type"]
    seat_type = SEAT_CODEX
    auth_file = save_auth_file(bundle)
...
def _run_post_register_relogin_oauth(
    ...
    *,
    ...
    bundle = registration.free_codex_oauth_bundle(bundle, email=email, force_email=True)
    auth_file = save_auth_file(bundle)
...
def _wait_for_direct_register_step(page, allowed_steps, timeout=15):
    ...
    while time.time() < deadline:
        ...
        if step == "passkey" and dismiss_passkey_prompt(page):
            time.sleep(0.5)
            continue
...
def _complete_direct_about_you(page):
    ...
    for attempt, values in enumerate(birthday_orders, 1):
        ...
        from autotoken.auth.invite import assert_not_blocked  # 局部导入避开循环
...
def reinvite_account(chatgpt_api, mail_client, acc):
    ...
    if access_token:
        try:
            try:
                from autotoken.interfaces.api import _auto_check_config
                from autotoken.settings.config import AUTO_CHECK_THRESHOLD
...
def _replace_single(chatgpt, mail_client, email, reason=""):
    ...
    email_lc = (email or "").lower()
    standby_list = rotation.standby_reuse_candidates(
        get_standby_accounts(),
        is_main_account_email=_is_main_account_email,
        recovered_only=True,
        exclude_email=email_lc,
    )
...
def cmd_rotate(target_seats=5):
    ...
    try:
        ...
        current_count, used_local_fallback, used_conservative_estimate = rotation.estimate_current_member_count(
            api_count=api_count,
            initial_api_count=initial_api_count,
        ...
        elif used_conservative_estimate:
            logger.info("[4/5] 成员数保守估算: %d（初始=%d，移出=%d）", current_count, initial_api_count, removed_now)
        vacancies = rotation.vacancy_count(target=TARGET, current_count=current_count)
        ...
        if vacancies <= 0:
            ...
            if excess > 0:
                ...
                local_active = rotation.overfill_cleanup_candidates(
                    load_accounts(),
                    is_main_account_email=_is_main_account_email,
...
def cmd_cleanup(max_seats=None):
    ...
    account_id = get_chatgpt_account_id()
    accounts = load_accounts()
    local_emails = team_cleanup.local_account_emails(accounts, is_main_account_email=_is_main_account_email)
    ...
    try:
        ...
        # 区分：本地管理的 vs 手动添加的
        local_members, external_members = team_cleanup.split_local_and_external_members(members, local_emails)
        ...
        # 确定要移除的数量
        max_seats_was_defaulted = max_seats is None
        to_remove_count, max_seats = team_cleanup.removal_count(total_members=total, max_seats=max_seats)
        if max_seats_was_defaulted:
        ...
        removable = team_cleanup.removable_members(
            local_members,
            accounts,
            exhausted_status=STATUS_EXHAUSTED,
        )
        ...
        if inv_result["status"] == 200:
            ...
            for inv in team_cleanup.pending_invites_for_local_accounts(invites, local_emails):
...
def main(argv=None):
    from autotoken.interfaces.cli import main as cli_main
