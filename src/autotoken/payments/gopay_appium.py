...
class GopayAppiumDriver:
    ...
    def __init__(
        ...
        *,
        ...
        # LDPlayer/雷电模拟器 配置
        self._ldconsole_path = ldconsole_path or _env_str("LDCONSOLE_PATH") or self._find_ldconsole()
        self._emulator_index = (
            emulator_index if emulator_index >= 0 else int(_env_str("GOPAY_APPIUM_EMULATOR_INDEX", "-1"))
        )
    ...
    def start_session(self, *, fresh: bool = True) -> None:
        ...
        try:
            ...
            raise GopayAppiumError("缺少 Appium 依赖，请安装: pip install Appium-Python-Client") from e
    ...
    def signup(
        ...
        *,
        ...
        while True:
            try:
                ...
                if self._is_technical_error(all_text):
                    raise
                if self._check_device_rate_limit(all_text):
                    raise GopayAppiumError("重启后触发设备频率限制") from exc
        ...
        pin_not_set = (
            "amankan saldomu" in all_text
            or "maksimalkan perlindunganmu" in all_text
            or "pasang pin" in all_text
            or "secure your balance" in all_text
            or "set up pin" in all_text
            or "maximize your protection" in all_text
        )
    ...
    def _navigate_to_signup(self) -> None:
        ...
        if not entry_found:
            ...
            phone_input = d.find_elements(
                "xpath", "//*[contains(@content-desc, 'Nomor HP') or contains(@content-desc, 'Phone')]"
            )
            if phone_input:
    ...
    def _switch_to_sms_otp(self, current_page_text: str = "") -> None:
        ...
        if (
            "pilih metode" in all_text
            or "choose method" in all_text
            or "select method" in all_text
            or "otp via sms" in all_text
        ):
        ...
        try:
            ...
            for el in descs:
                try:
                    ...
                    if (
                        el.get_attribute("clickable") == "true"
                        and desc
                        and "whatsapp" not in desc
                        and "back" not in desc
                        and "dismiss" not in desc
                    ):
                        el.click()
    ...
    def _setup_pin_from_home(
        self, pin: str, otp_provider: Callable[[str], str], pre_pin_otp_hook: Callable[[], None] | None = None
    ) -> None:
        """从主页导航到 PIN 设置并完成设置。
        ...
        while pin_setup_retry < 2:
            ...
            if ("hindari" in all_text or "avoid" in all_text) and (
                "berulang" in all_text or "berurut" in all_text or "repeating" in all_text or "sequential" in all_text
            ):
        ...
        if any(
            ind in all_text
            for ind in ["otp", "verifikasi", "verification", "kode", "code", "masukkan otp", "enter otp"]
        ):
        ...
        if self._is_home_screen(all_text):
            ...
            "berhasil" in all_text
            or "selesai" in all_text
            or "sukses" in all_text
            or "success" in all_text
            or "done" in all_text
            or "completed" in all_text
        ):
        ...
        try:
            ...
            for path in prefs_paths:
                content = self._adb_shell(
                    f"su -c 'cat {path}' 2>/dev/null || run-as {GOPAY_PACKAGE} cat {path.split('/shared_prefs/')[1]} 2>/dev/null"
                )
    ...
    def _reset_via_ldconsole(self) -> None:
        ...
        self._run_ldconsole(
            "modify",
            f"--index {idx} --imei auto --imsi auto --simserial auto --androidid auto --mac auto",
        )
        ...
        while time.time() < deadline:
            try:
                ...
                if (
                    self.adb_serial
                    and self.adb_serial in result.stdout
                    and "device" in result.stdout.split(self.adb_serial)[1].split("\n")[0]
                ):
                    break
    ...
    def _run_ldconsole(self, command: str, args: str = "") -> str:
        """执行 ldconsole 命令。"""
        cmd = self._ldconsole_command_args(self._ldconsole_path, command, args)
        if not cmd:
            return ""
        try:
            proc = subprocess.run(
    ...
    @staticmethod
    def _ldconsole_command_args(path: str, command: str, args: str = "") -> list[str]:
        executable = str(path or "").strip()
        action = str(command or "").strip()
        if not executable or not action:
            return []
        lexer = shlex.shlex(str(args or ""), posix=False)
        lexer.whitespace_split = True
        lexer.commenters = ""
        extra = [part.strip().strip("\"'") for part in lexer if part.strip().strip("\"'")]
        return [executable, action, *extra]
            ...
            try:
                ...
                for el in elements:
                    ...
                    try:
                        ...
                        pass
                    if (el.get_attribute("clickable") == "true" or el.tag_name == "android.widget.Button") and str(
                        enabled
                    ).lower() != "false":
                        el.click()
