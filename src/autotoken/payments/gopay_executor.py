...
_CurlCffiSession = payment_http_service._CurlCffiSession  # compatibility for existing tests/monkeypatches
...
CHECKOUT_ERROR_PATTERNS = payment_checkout_state_service.CHECKOUT_ERROR_PATTERNS
CHECKOUT_PAYMENT_NOT_APPROVED_PATTERNS = payment_checkout_state_service.CHECKOUT_PAYMENT_NOT_APPROVED_PATTERNS
    ...
    *,
    ...
    try:
        ...
        if require_curl_cffi and message.startswith("ChatGPT checkout/approve"):
            message = f"GoPay {message}"
        raise GoPayFlowError(message, stage=getattr(exc, "stage", "chatgpt_http_session")) from exc
    finally:
        payment_http_service._CurlCffiSession = original_curl_cffi_session


def _http_transport_name(http: Any) -> str:
    return payment_http_service.http_transport_name(http)
...
def _parse_display_amount(value: Any) -> int | None:
    return payment_checkout_state_service.parse_display_amount(value)
...
def _redact_network_capture_text(text: Any, *, limit: int = 900) -> str:
    ...
    if not raw:
        return ""
    raw = re.sub(
        r"\b(reference_id|reference|token|client_secret|otp|pin|password)=([^&\s]+)",
        r"\1=<redacted>",
        raw,
        flags=re.IGNORECASE,
    )
    raw = re.sub(
        r'"(reference_id|reference|token|client_secret|otp|pin|password)"\s*:\s*"[^"]*"',
        r'"\1":"<redacted>"',
        raw,
        flags=re.IGNORECASE,
    )
...
def _gopay_progress_message(stage: str, payload: dict[str, Any]) -> str:
    ...
    if stage == "gopay_sms_channel_switch_failed":
        ...
        return (
            f"GoPay SMS OTP 切换失败，回退重发流程：{_compact_log_text(reason, limit=120)}"
            if reason
            else "GoPay SMS OTP 切换失败，回退重发流程"
        )
    if stage == "sms_otp_resend_failed":
        reason = str(payload.get("reason") or "").strip()
        return (
            f"GoPay SMS OTP 重新发送失败，继续等待接码接口：{_compact_log_text(reason, limit=120)}"
            if reason
            else "GoPay SMS OTP 重新发送失败，继续等待接码接口"
        )
    if stage == "sms_provider_resend_failed":
        reason = str(payload.get("reason") or "").strip()
        return (
            f"短信平台重新接收 GoPay OTP 失败，继续等待接码接口：{_compact_log_text(reason, limit=120)}"
            if reason
            else "短信平台重新接收 GoPay OTP 失败，继续等待接码接口"
        )
...
def _is_chatgpt_approve_blocked_result(result: dict | None) -> bool:
    return payment_results_service.is_chatgpt_approve_blocked_result(result)


def _is_checkout_payment_not_approved_error(text: str) -> bool:
    return payment_checkout_state_service.is_checkout_payment_not_approved_error(text)


def _is_checkout_customer_location_error(text: str) -> bool:
    return payment_checkout_state_service.is_checkout_customer_location_error(text)


def _is_checkout_rate_limited_error(text: str) -> bool:
    return payment_checkout_state_service.is_checkout_rate_limited_error(text)


def _is_checkout_payment_not_approved_result(result: dict | None) -> bool:
    return payment_checkout_state_service.is_checkout_payment_not_approved_result(result)


def _is_gopay_payment_process_rotatable_result(result: dict | None) -> bool:
    return payment_results_service.is_gopay_payment_process_rotatable_result(result)


def _is_gopay_nonzero_amount_blocked_result(result: dict | None) -> bool:
    return payment_results_service.is_gopay_nonzero_amount_blocked_result(result)


def _is_gopay_already_linked_result(result: dict | None) -> bool:
    return payment_results_service.is_gopay_already_linked_result(result)


def _looks_like_chatgpt_user_paid_text(text: str) -> bool:
    return payment_results_service.looks_like_chatgpt_user_paid_text(text)


def _is_chatgpt_user_paid_result(result: dict | None) -> bool:
    return payment_results_service.is_chatgpt_user_paid_result(result)


def _as_chatgpt_user_paid_success(
    result: dict | None, *, checkout_url: str = "", billing_info: dict | None = None
) -> dict:
    return payment_results_service.chatgpt_user_paid_success(
        result,
...
def _is_midtrans_linking_rate_limited_result(result: dict | None) -> bool:
    return payment_results_service.is_midtrans_linking_rate_limited_result(result)
...
def _is_chatgpt_token_invalidated_result(result: dict | None) -> bool:
    return payment_results_service.is_chatgpt_token_invalidated_result(result)


def _looks_like_http_forbidden_text(text: str) -> bool:
    return payment_results_service.looks_like_http_forbidden_text(text)


def _gopay_pending_retry_reason(result: dict | None) -> str:
    return payment_results_service.gopay_pending_retry_reason(result)


def _gopay_pending_retry_source_stage(result: dict | None, reason: str) -> str:
    return payment_results_service.gopay_pending_retry_source_stage(result, reason)
...
def _looks_like_gopay_rate_limit_text(text: str) -> bool:
    return payment_results_service.looks_like_gopay_rate_limit_text(text)
...
def _chatgpt_cookie_header(session_token: str = "", account_id: str = "", device_id: str = "") -> str:
    return chatgpt_session_service.chatgpt_cookie_header(session_token, account_id, device_id)
...
def _chatgpt_reference_cookie_header(
    ...
    return chatgpt_session_service.chatgpt_reference_cookie_header(
...
def _merge_cookie_headers(*headers: str) -> str:
    return chatgpt_session_service.merge_cookie_headers(*headers)


def _cookie_header_from_http_session(http: Any) -> str:
    return chatgpt_session_service.http_session_cookie_header(http)
...
def _extract_auth_session_context(
    ...
    *,
    ...
    session = auth_session if isinstance(auth_session, dict) else None
    file_context = auth_file_context if isinstance(auth_file_context, dict) else None
    return chatgpt_session_service.extract_auth_session_context(
        email,
        load_session=(lambda _email: session) if session is not None else load_auth_session,
        auth_file_context=file_context if file_context is not None else _load_chatgpt_auth_file_context(email),
    )
...
def _gopay_signup_bridge_resend_url(sms_url: str) -> str:
    return sms_otp_service.gopay_signup_bridge_resend_url(sms_url)


def _normalize_local_gopay_signup_bridge_url(sms_url: str) -> str:
    return sms_otp_service.normalize_local_gopay_signup_bridge_url(sms_url)


def _trigger_gopay_signup_bridge_resend(sms_url: str) -> bool:
    return sms_otp_service.trigger_gopay_signup_bridge_resend(sms_url, http_get=requests.get)
...
def _poll_otp_from_sms_url(
    ...
    *,
    ...
    return sms_otp_service.poll_otp_from_sms_url(
        ...
        fetch_sms_code_fn=lambda url, **kwargs: _fetch_sms_code(url, **kwargs),
        bridge_resend_url_fn=_gopay_signup_bridge_resend_url,
        trigger_bridge_resend_fn=_trigger_gopay_signup_bridge_resend,
        sleep_fn=time.sleep,
        time_fn=time.time,
...
def _playwright_cookie_items_from_header(cookie_header: str) -> list[dict[str, str]]:
    return chatgpt_session_service.playwright_cookie_items_from_header(cookie_header)
...
def _generate_id_checkout_http(
    ...
    *,
    ...
    if (
        ...
        logger.info(
            "[gopay_executor] hosted checkout ui mode unsupported, retrying checkout generation with custom mode"
        )
        resp = http.post(
            "https://chatgpt.com/backend-api/payments/checkout",
...
def _chatgpt_approve_blocked_message(payload: dict) -> str:
    return payment_results_service.chatgpt_approve_blocked_message(payload)
...
def _verify_checkout_http(
    ...
    *,
    ...
    return chatgpt_session_service.chatgpt_verify_checkout_result(resp.status_code, resp.text or "")
...
def _collect_page_cookie_header(api: ChatGPTTeamAPI) -> str:
    try:
        ...
        cookies = []
    return chatgpt_session_service.cookie_header_from_cookie_items(cookies)
...
def _approve_checkout_in_page(
    ...
    *,
    ...
    if not result.get("ok"):
        detail = (
            result.get("error") or (result.get("data") or {}).get("detail") or (result.get("data") or {}).get("error")
        )
        raise GoPayFlowError(
    ...
    *,
    ...
    def click_consent_if_present() -> bool:
        ...
        for selector in selectors:
            try:
                ...
                record(
                    {
                        ...
                        "selector": selector,
                        "text": _compact_log_text(text, limit=80),
                    }
                )
                ...
                record(
                    {
                        "type": "marker",
                        "action": "after_click_consent",
                        "page_url": _safe_url_summary(getattr(page, "url", "")),
                    }
                )
                return True
            except Exception:
        ...
        record(
            {
                "type": "marker",
                "action": "click_consent_not_found",
                "page_url": _safe_url_summary(getattr(page, "url", "")),
            }
        )
        return False
...
def _browser_checkout_to_gopay_redirect(
    ...
    *,
    ...
    home_ready = _goto_with_retry(
        api.page, "https://chatgpt.com/", wait_until="domcontentloaded", timeout=60000, attempts=3
    )
    if home_ready:
        try:
    ...
    while time.time() < deadline:
        ...
        if error:
            ...
            if _is_checkout_customer_location_error(error):
                ...
                ok, fill_error = _fill_billing_form_on_page(
                    api, billing, session_id, screenshot_paths, progress=progress
                )
                if not ok:
...
class GoPayHttpCharger:
    ...
    def __init__(
        ...
        *,
        ...
        self.midtrans_client_id = str(
            midtrans_client_id or os.environ.get("GOPAY_MIDTRANS_CLIENT_ID") or DEFAULT_MIDTRANS_CLIENT_ID
        ).strip()
    ...
    def _stripe_create_payment_method(
        self, checkout_session_id: str, stripe_pk: str, init_ctx: dict | None = None
    ) -> str:
    ...
    def _stripe_elements_session(self, checkout_session_id: str, stripe_pk: str, init_ctx: dict) -> dict:
        ...
        resp = self._request(
            "get", f"{STRIPE_API}/v1/elements/sessions", params=params, stage="stripe_elements_session"
        )
        if resp.status_code != 200:
            logger.info(
    ...
    def _stripe_confirm(
        self, checkout_session_id: str, payment_method_id: str, stripe_pk: str, init_ctx: dict | None = None
    ) -> dict:
    ...
    def _confirm_state_summary(payload: dict) -> str:
        ...
        submission_attempt = (
            payload.get("submission_attempt") if isinstance(payload.get("submission_attempt"), dict) else {}
        )
    ...
    def _midtrans_init_linking(self, snap_token: str) -> str:
        ...
        for attempt in range(1, GOPAY_LINK_RETRY_LIMIT + 2):
            ...
            if resp.status_code == 406:
                ...
                continue
            raise GoPayFlowError(
                f"Midtrans linking 失败: HTTP {resp.status_code} {(resp.text or '')[:300]}", stage="midtrans_linking"
            )
    ...
    def _gopay_validate_reference(self, reference_id: str):
        ...
        if _looks_like_gopay_rate_limit_payload(payload):
            ...
            logger.info(
                "[gopay_executor] GoPay validate-reference returned rate-limit payload: %s",
    ...
    def _gopay_user_consent(self, reference_id: str):
        ...
        if _looks_like_gopay_rate_limit_payload(payload):
            self._progress("gopay_rate_limited")
            logger.info(
                "[gopay_executor] GoPay user-consent returned rate-limit payload: %s", _safe_error_summary(payload)
            )
            raise GoPayRateLimited(_gopay_rate_limited_message(), stage="gopay_rate_limited")
        if not payload.get("success"):
    ...
    def _gopay_switch_to_sms_otp(self, reference_id: str) -> bool:
        ...
        try:
            ...
            if _looks_like_gopay_rate_limit_payload(payload):
                self._progress("gopay_rate_limited")
                logger.info(
                    "[gopay_executor] GoPay SMS channel switch returned rate-limit payload: %s",
                    _safe_error_summary(payload),
                )
                raise GoPayRateLimited(_gopay_rate_limited_message(), stage="gopay_rate_limited")
            if not payload.get("success"):
        ...
        logger.info(
            "[gopay_executor] GoPay OTP switched to SMS via user-consent: reference=%s", _mask_log_value(reference_id)
        )
        return True
    ...
    def _gopay_resend_otp(self, reference_id: str):
        ...
        if _looks_like_gopay_rate_limit_payload(payload):
            self._progress("gopay_rate_limited")
            logger.info(
                "[gopay_executor] GoPay resend-otp returned rate-limit payload: %s", _safe_error_summary(payload)
            )
            raise GoPayRateLimited(_gopay_rate_limited_message(), stage="gopay_rate_limited")
        if not payload.get("success"):
        ...
        self._progress("sms_otp_triggered")
        logger.info(
            "[gopay_executor] GoPay OTP triggered via protocol resend: reference=%s", _mask_log_value(reference_id)
        )
    ...
    def _gopay_validate_otp(self, reference_id: str, otp: str) -> tuple[str, str]:
        ...
        if _looks_like_gopay_rate_limit_payload(payload):
            self._progress("gopay_rate_limited")
            logger.info(
                "[gopay_executor] GoPay validate-otp returned rate-limit payload: %s", _safe_error_summary(payload)
            )
            raise GoPayRateLimited(_gopay_rate_limited_message(), stage="gopay_rate_limited")
        if _looks_like_gopay_invalid_otp_payload(payload):
            raise GoPayOTPInvalid(f"GoPay OTP 错误: {payload}", stage="gopay_validate_otp")
    ...
    def run_from_snap_token(self, *, snap_token: str, checkout_session_id: str = "") -> dict:
        ...
        if self.otp_channel == "sms":
            try:
                self.otp_provider._gopay_resend_callback = lambda: self._gopay_resend_otp(reference_id)
            except Exception:
                pass
        ...
        for attempt in range(1, max_otp_attempts + 1):
            try:
                self.otp_provider._gopay_ignored_otps = set(invalid_otps)
            except Exception:
                pass
...
def _scroll_locator_into_view(locator, label: str):
    return payment_form_fields_service.scroll_locator_into_view(
        locator,
        label,
        logger=logger,
        log_prefix="[gopay_executor]",
    )


def _read_locator_value(locator) -> str:
    return payment_form_fields_service.read_locator_value(locator)
...
def _locator_by_placeholder_or_label(
    api: ChatGPTTeamAPI, placeholders: list[str], labels: list[str], timeout_ms: int = 1200, frames=None
):
    return payment_form_fields_service.locator_by_placeholder_or_label(
        frames or _iter_page_frames(api),
        placeholders,
        labels,
...
def _locator_by_placeholder_or_label_with_state(
    ...
    return payment_form_fields_service.locator_by_placeholder_or_label_with_state(
        frames or _iter_page_frames(api),
        placeholders,
        labels,
        timeout_ms=timeout_ms,
...
def _resolve_page_billing_locator(
    ...
    return payment_form_fields_service.resolve_locator_in_frames(
        frames or _iter_page_frames(api),
        selectors,
...
def _score_billing_frame(frame) -> int:
    return payment_form_fields_service.score_billing_frame(frame)


def _find_billing_form_frames(api: ChatGPTTeamAPI, timeout_seconds: int = 5):
    return payment_form_fields_service.find_billing_form_frames(
        lambda: _iter_page_frames(api),
        timeout_seconds=timeout_seconds,
        logger=logger,
        log_prefix="[gopay_executor]",
...
def _scroll_to_billing_section(api: ChatGPTTeamAPI):
    return payment_form_fields_service.scroll_to_billing_section(
        api.page,
        visible_locator=lambda selectors, timeout_ms: _visible_locator(api, selectors, timeout_ms=timeout_ms),
        logger=logger,
        log_prefix="[gopay_executor]",
    )
...
def _fill_billing_form_on_page(
    ...
    def _fill_page(
        ...
        return payment_form_fields_service.fill_billing_page_field(
            selectors=selectors,
            value=value,
            label=label,
            optional=optional,
            ...
            placeholders=placeholders,
            labels=labels,
            resolve_locator=_resolve_field_locator,
            capture_screenshot=lambda stage: _capture_screenshot(api, session_id, stage, screenshot_paths),
            scroll_locator=_scroll_locator_into_view,
            progress=progress,
            logger=logger,
            log_prefix="[gopay_executor]",
            ...
            looks_like_phone_number=_looks_like_phone_number,
        )
    ...
    def _select_page(
        ...
        return payment_form_fields_service.select_billing_page_field(
            selectors=selectors,
            value=value,
            label=label,
            optional=optional,
    ...
    def _verify_billing_stable_before_submit():
        return payment_form_fields_service.verify_billing_fields_stable(
            field_locators,
            billing,
            suppress_address_autocomplete_ui=lambda: _suppress_address_autocomplete_ui(api),
            dismiss_address_autocomplete=lambda locator: _dismiss_address_autocomplete(api, locator),
            capture_screenshot=lambda stage: _capture_screenshot(api, session_id, stage, screenshot_paths),
            progress=progress,
            logger=logger,
            log_prefix="[gopay_executor]",
...
def _extract_sms_codes(text: str) -> list[str]:
    return sms_otp_service.extract_sms_codes(text)


def _extract_sms_code(text: str) -> str:
    return sms_otp_service.extract_sms_code(text)


def _fetch_sms_code(sms_url: str, ignored_otps: set[str] | None = None) -> str:
    return sms_otp_service.fetch_sms_code(
        sms_url,
        ignored_otps=ignored_otps,
...
def _phone_page_ready(api: ChatGPTTeamAPI) -> bool:
    ...
    has_phone_hints = any(
        hint in body for hint in ("nomor handphone", "phone number", "nomor telepon", "whatsapp", "otp")
    )
    still_on_billing = any(
        hint in body for hint in ("账单地址", "billing address", "全名", "地址第 1 行", "address line 1")
    )
...
def _compact_checkout_error(text: str) -> str:
    return payment_checkout_state_service.compact_checkout_error(text)
...
def _run_gopay_bind_task_once(
    *,
    ...
    session_context = _extract_auth_session_context(
        email,
        auth_session=auth_session,
        auth_file_context=auth_file_context,
    )
    ...
    if not all(str(billing.get(key) or "").strip() for key in ("name", "country", "state", "city", "zip", "address1")):
        billing = _fetch_random_billing_address()
        progress(
            "billing_address_generated", billing_city=billing.get("city", ""), billing_state=billing.get("state", "")
        )
    ...
    try:
        ...
        device_id = session_context.get("device_id", "") or str(uuid.uuid4())
        openai_sentinel_token = session_context.get("openai_sentinel_token", "")
        oai_client_version = session_context.get("oai_client_version", "")
        oai_client_build_number = session_context.get("oai_client_build_number", "")
        token_source = (
            "auth_session"
            if str(auth_session.get("accessToken") or auth_session.get("access_token") or "").strip()
            else "auth_file"
        )
        ...
        if not access_token:
            return _build_result(
                ...
                failure_stage="generate_checkout",
                message=(
                    "GoPay 协议模式不会启动 Playwright；"
                    f"账号缺少 ChatGPT access_token；已尝试 auth_session 和 auth_file/CPA 认证文件 (source={token_source})"
                ),
                screenshot_paths=screenshot_paths,
                billing_info=public_billing_info,
            )
        ...
        if direct_redirect_mode:
            ...
            if not final_checkout_url:
                ...
                try:
                    ...
                    raw_checkout = (
                        generated_checkout_meta.get("raw")
                        if isinstance(generated_checkout_meta.get("raw"), dict)
                        else {}
                    )
                    logger.info(
                        "[gopay_executor] generated GoPay checkout session for browser UI: %s",
            ...
            raw_checkout = (
                generated_checkout_meta.get("raw") if isinstance(generated_checkout_meta.get("raw"), dict) else {}
            )
        else:
        ...
        if flow_state in {"succeeded", "verify_timeout"}:
            ...
            result = _build_result(
                "success",
                message=(
                    "GoPay 支付已完成，ChatGPT verify 网络超时，已按成功处理" if verify_timeout else "GoPay 支付成功"
                ),
                screenshot_paths=screenshot_paths,
                checkout_url=final_checkout_url,
                billing_info=public_billing_info,
        ...
        return _build_result(
            ...
            message=f"执行 GoPay 任务时出现异常: {exc}",
            screenshot_paths=screenshot_paths,
            checkout_url=final_checkout_url,
            billing_info=public_billing_info,
        )
...
def run_gopay_bind_task(
    *,
    ...
    final_checkout_url = str(checkout_url or "").strip()
    checkout_ui_mode = _normalize_checkout_ui_mode(checkout_ui_mode)
    pending_retry_attempts = gopay_pending_retry_service.normalize_pending_retry_attempts(pending_retry_attempts)
    ...
    for raw_phone_account in phone_accounts or []:
        ...
        account_country_code = str(
            raw_phone_account.get("country_code") or raw_phone_account.get("countryCode") or country_code or ""
        ).strip()
        account_phone_number = str(
            raw_phone_account.get("phone_number") or raw_phone_account.get("phoneNumber") or ""
        ).strip()
        ...
        account_otp_channel = (
            str(raw_phone_account.get("otp_channel") or raw_phone_account.get("otpChannel") or otp_channel or "sms")
            .strip()
            .lower()
        )
        if account_otp_channel == "whatsapp":
            account_sms_url = (
                str(os.environ.get("AUTOTOKEN_LOCAL_BASE_URL") or "http://127.0.0.1:8787").strip().rstrip("/")
                + "/otp/whatsapp/latest"
            )
    ...
    def emit_payload(payload: dict[str, Any]):
    ...
    explicit_candidates = [
        str(candidate or "").strip().lower() for candidate in (account_emails or []) if str(candidate or "").strip()
    ]
    rotation_enabled = (
    ...
    def queue_pending_retry(candidate: str, *, reason: str, stage: str, retry_round: int = 0):
        ...
        emit_payload(
            gopay_pending_retry_service.pending_retry_queued_progress(
                email=candidate,
    ...
    def emit_pending_retry_failed(
        ...
        *,
        ...
        if message is not None:
            payload_result["message"] = message
        emit_payload(
            gopay_pending_retry_service.pending_retry_failed_progress(
                email=candidate,
                retry_round=retry_round,
                max_retry_rounds=pending_retry_attempts,
                result=payload_result,
                reason=reason,
                default_message="GoPay 待重试失败",
    ...
    def attach_common_lists(result: dict):
        ...
        result["auth_session_refresh_failed_emails"] = [
            item for item in auth_session_refresh_failed if item not in success_set
        ]
    ...
    for index, candidate in enumerate(candidates, 1):
        ...
        if (
            _is_chatgpt_token_invalidated_result(result)
            and pending_retry_attempts > 0
            and refresh_auth_session_for_retry(candidate, result)
        ):
    ...
    for retry_round in range(1, pending_retry_attempts + 1):
        ...
        if not retry_candidates:
            break
        wait_seconds = gopay_pending_retry_service.pending_retry_wait_seconds(retry_round)
        logger.info(
        ...
        emit_payload(
            gopay_pending_retry_service.pending_retry_wait_progress(
                retry_round=retry_round,
                max_retry_rounds=pending_retry_attempts,
        ...
        emit_payload(
            gopay_pending_retry_service.pending_retry_started_progress(
                retry_round=retry_round,
                max_retry_rounds=pending_retry_attempts,
                pending_retry=len(retry_candidates),
            )
        ...
        for retry_offset, candidate in enumerate(retry_candidates, 1):
            ...
            emit_payload(
                gopay_pending_retry_service.pending_retry_account_progress(
                    email=candidate,
            ...
            if _is_chatgpt_token_invalidated_result(result):
                remove_email(blocked, candidate)
                remove_email(skipped_cooldown, candidate)
                if retry_round < pending_retry_attempts and refresh_auth_session_for_retry(
                    candidate, result, retry_round=retry_round
                ):
                    emit_pending_retry_failed(
                        candidate,
                        retry_round=retry_round,
            ...
            if _is_checkout_payment_not_approved_result(result):
                ...
                last_rejected_result = result
                last_rejected_email = candidate
                emit_pending_retry_failed(candidate, retry_round=retry_round, result=result)
                continue
            if _is_gopay_payment_process_rotatable_result(result):
                remove_email(blocked, candidate)
                remove_email(skipped_cooldown, candidate)
    ...
    if (
        last_rejected_result
        and not last_blocked_result
        and not last_payment_failed_result
        and not last_nonzero_blocked_result
    ):
    ...
    if last_payment_failed_result and not last_blocked_result and not last_nonzero_blocked_result:
        ...
        last_payment_failed_result["email_used"] = last_payment_failed_email or (
            attempted[-1] if attempted else requested_email
        )
    ...
    if last_nonzero_blocked_result and not last_blocked_result:
        ...
        last_nonzero_blocked_result["email_used"] = last_nonzero_blocked_email or (
            attempted[-1] if attempted else requested_email
        )
