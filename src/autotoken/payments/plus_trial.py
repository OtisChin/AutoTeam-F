...
class CheckoutExtractor:
    ...
    def _stage_proxies(self) -> tuple[str, str]:
        ...
        if not checkout_proxy and not update_proxy and not has_cliproxy_config:
            return "", ""
...
def _payload_billing_value(payload: dict[str, Any], key: str, default: str) -> str:
    billing = payload.get("billing_details") if isinstance(payload.get("billing_details"), dict) else {}
    return str(billing.get(key) or default).strip().upper() or default
...
def generate_plus_trial_checkout_link(
    access_token: str,
    ...
    *,
    ...
    """Generate a Plus trial checkout link using Simon's checkout/update promo flow."""

    payload = payload or {}
    checkout_proxy = _payload_value(payload, "checkout_proxy") or _env_value("PLUS_TRIAL_CHECKOUT_PROXY")
    update_proxy = _payload_value(payload, "update_proxy") or _env_value("PLUS_TRIAL_UPDATE_PROXY")
    skip_proxy_check = _env_value("PLUS_TRIAL_SKIP_PROXY_CHECK").lower() in {"1", "true", "yes", "on"}
    using_direct_connection = not checkout_proxy and not update_proxy
    credentials = Credentials(
        access_token=str(access_token or "").strip(),
        session_cookie=str(payload.get("session_cookie") or ""),
    )
    config = ExtractorConfig(
        billing_country=_payload_billing_value(payload, "country", "PH"),
        currency=_payload_billing_value(payload, "currency", "PHP"),
        payment_locale=str(payload.get("payment_locale") or "en"),
        checkout_proxy_country=(
            _payload_value(payload, "checkout_proxy_country") or _env_value("PLUS_TRIAL_CHECKOUT_PROXY_COUNTRY", "US")
        ).upper()
        or "US",
        update_proxy_country=(
            _payload_value(payload, "update_proxy_country") or _env_value("PLUS_TRIAL_UPDATE_PROXY_COUNTRY", "TR")
        ).upper()
        or "TR",
        cliproxy_host=_env_value("CLIPROXY_HOST", _env_value("OPENAI_PAY_CLIPROXY_HOST", "127.0.0.1:19010")),
        cliproxy_username=_env_value("CLIPROXY_USERNAME"),
        cliproxy_password=_env_value("CLIPROXY_PASSWORD"),
        checkout_proxy=checkout_proxy,
        update_proxy=update_proxy,
        plan_name=str(payload.get("plan_name") or "chatgptplusplan"),
        promo_campaign_id=_env_value("PLUS_TRIAL_PROMO_CAMPAIGN_ID", "plus-1-month-free") or "plus-1-month-free",
        checkout_ui_mode=str(payload.get("checkout_ui_mode") or "custom"),
        timeout=float(_env_value("PLUS_TRIAL_TIMEOUT_SECONDS", "45.0") or 45.0),
        checkout_attempts=int(_env_value("PLUS_TRIAL_CHECKOUT_ATTEMPTS", "10") or 10),
        update_attempts=int(_env_value("PLUS_TRIAL_UPDATE_ATTEMPTS", "15") or 15),
        full_attempts=int(_env_value("PLUS_TRIAL_FULL_ATTEMPTS", "3") or 3),
        cf_same_identity_attempts=int(_env_value("PLUS_TRIAL_CF_SAME_IDENTITY_ATTEMPTS", "5") or 5),
        cf_retry_delay=max(0.0, float(_env_value("PLUS_TRIAL_CF_RETRY_DELAY_SECONDS", "3.0") or 3.0)),
        verify_proxy_country=_payload_bool(payload, "verify_proxy_country", not skip_proxy_check and not using_direct_connection),
    )
    result = CheckoutExtractor(
        credentials,
        config,
    ...
    return {
        ...
        "chatgpt_checkout_url": result.long_url,
        "long_url": result.long_url,
        "billing_country": result.billing_country,
        "currency": result.currency,
        "payment_locale": result.payment_locale,
