...
def paypal_nonzero_blocked_pool_emails(result: dict, actual_email: str) -> list[str]:
    seen = set()
    emails = []
    for raw_email in result.get("nonzero_blocked_emails") or []:
        email = _normalized_email(raw_email)
...
def paypal_pending_retry_reason(result: dict | None) -> str:
    if not isinstance(result, dict) or result.get("status") == "success":
        return ""
...
def paypal_pending_retry_source_stage(result: dict | None, reason: str) -> str:
