...
from autotoken.settings.admin_state import get_chatgpt_account_id
from autotoken.storage.accounts import find_account, load_accounts, save_accounts
from autotoken.storage.auth_files import is_inside_auth_dir
from autotoken.storage.auth_storage import AUTH_DIR

logger = logging.getLogger(__name__)


def _local_auth_candidates(email: str, acc: dict | None) -> set[Path]:
    email_l = normalized_email(email)
    candidates: set[Path] = set()

    if acc and acc.get("auth_file"):
        auth_file = Path(acc["auth_file"])
        if is_inside_auth_dir(auth_file, auth_dir=AUTH_DIR):
            candidates.add(auth_file)

    if AUTH_DIR.exists():
        prefix = f"codex-{email_l}-"
        for path in AUTH_DIR.glob("codex-*.json"):
            if path.name.lower().startswith(prefix) and is_inside_auth_dir(path, auth_dir=AUTH_DIR):
                candidates.add(path)
...
def delete_managed_account(
    ...
    *,
    ...
    try:
        ...
        for path in sorted(_local_auth_candidates(email, acc)):
            if path.exists():
                path.unlink()
        ...
        if acc:
            ...
            if remove_cloudmail and cloudmail_account_id:
                try:
                    ...
                    logger.warning("[账号] 删除临时邮箱账户失败: %s", exc)
