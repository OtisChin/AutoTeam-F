"""Persistent Outlook account-pool usage state."""
...
STATE_FILE = PROJECT_ROOT / "data" / "outlook_pool.json"
_NAMESPACE = "outlook_pool"
...
def _db_path() -> Path:
    try:
        if Path(STATE_FILE).resolve() != (PROJECT_ROOT / "data" / "outlook_pool.json").resolve():
...
def _load_records() -> dict[str, dict[str, Any]]:
    ...
    if isinstance(raw, dict):
        ...
        for key, value in raw.items():
            ...
            if not email:
                continue
            records[email] = dict(value) if isinstance(value, dict) else {"registered_at": 0, "source": str(value or "")}
        return records
