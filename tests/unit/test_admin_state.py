...
from autotoken import admin_state


def test_load_admin_state_ignores_oversized_state_file(tmp_path, monkeypatch):
    state_file = tmp_path / "state.json"
    legacy_file = tmp_path / "session"
    state_file.write_text("x" * (admin_state.ADMIN_STATE_FILE_MAX_BYTES + 1), encoding="utf-8")
    ...
    assert state_file.stat().st_size == admin_state.ADMIN_STATE_FILE_MAX_BYTES + 1
...
def test_load_admin_state_ignores_oversized_legacy_session_file(tmp_path, monkeypatch):
    state_file = tmp_path / "state.json"
    legacy_file = tmp_path / "session"
    legacy_file.write_text("x" * (admin_state.ADMIN_STATE_FILE_MAX_BYTES + 1), encoding="utf-8")
