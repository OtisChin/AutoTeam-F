"""测试 cloud-mail provider。

只测能离线复现的纯逻辑路径(字段映射、createTime 解析、accountId 解析、
auth header 形态)。涉及实际 e2e 的待验证项见 cloud_mail.py 的 TODO(cloud-mail-verify)。
"""
...
from autotoken.mail import cloud_mail as mod
...
def test_auth_header_uses_raw_jwt_without_bearer_prefix(monkeypatch):
    """已现场验证:cloud-mail security.js 不解析 Bearer 前缀,直接读 Authorization。"""
...
def test_normalize_mail_record_maps_cloud_mail_fields(monkeypatch):
    client = _make_client(monkeypatch)
    row = {
        "emailId": 411,
        "accountId": 43,
...
def test_normalize_mail_record_falls_back_to_message_field(monkeypatch):
    """cloud-mail 部分版本 HTML 在 message 字段而非 content,需要兜底。"""
    client = _make_client(monkeypatch)
    row = {
        "emailId": 1,
        "subject": "Hi",
...
def test_login_requires_url_and_credentials(monkeypatch):
    monkeypatch.delenv("CLOUD_MAIL_API_URL", raising=False)
...
def test_list_emails_passes_type_zero_to_avoid_empty_response(monkeypatch):
    """cloud-mail service/email-service.js list() 把 type 当 IS NULL 处理时,所有 RECEIVE
...
def test_list_accounts_paginates_past_server_size_cap(monkeypatch):
    """cloud-mail account-service.js list() 服务端硬 cap 30 条。请求 size=200 必须循环翻页。"""
    client = _make_client(monkeypatch)
    pages = [
        # page 1: 30 条
