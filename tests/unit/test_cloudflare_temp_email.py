"""测试 cloudflare_temp_email provider。
...
from autotoken.mail import cloudflare_temp_email
...
def test_list_accounts_maps_admin_address_results(monkeypatch):
    client = cloudflare_temp_email.CloudflareTempEmailClient()
    row = {
        "id": 123,
        "name": "tmp-user@example.com",
        "password": "secret",
        "created_at": 1761331200,
        "updated_at": 1761331210,
        "mail_count": 2,
        "send_count": 1,
        "source_meta": {"source": "admin"},
    }
    ...
    assert client.list_accounts(size=10) == [
        {
            "accountId": 123,
            "email": "tmp-user@example.com",
            "password": "secret",
            "createTime": 1761331200,
            "updateTime": 1761331210,
            "mailCount": 2,
            "sendCount": 1,
            "sourceMeta": {"source": "admin"},
            "raw": row,
        }
    ]
...
def test_factory_returns_cloudflare_temp_email_by_default(monkeypatch):
    """兼容入口仍能返回默认 provider。"""
    monkeypatch.delenv("MAIL_PROVIDER", raising=False)
    from autotoken.cloudmail import CloudMailClient

    client = CloudMailClient()
    assert isinstance(client, cloudflare_temp_email.CloudflareTempEmailClient)


def test_factory_dispatches_to_cloud_mail_when_configured(monkeypatch):
    monkeypatch.setenv("MAIL_PROVIDER", "cloud-mail")
    monkeypatch.setenv("CLOUD_MAIL_API_URL", "http://example.com")
    from autotoken.cloudmail import CloudMailClient
    from autotoken.mail.cloud_mail import CloudMailProviderClient

    client = CloudMailClient()
    assert isinstance(client, CloudMailProviderClient)
