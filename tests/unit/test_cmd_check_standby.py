...
from autotoken.accounts import STATUS_ACTIVE, STATUS_AUTH_INVALID, STATUS_STANDBY
...
def _stub_cmd_check_deps(monkeypatch, accounts_list):
    ...
    # 注册域名走 config import,无需额外 mock


def _fake_auth_file(tmp_path, email):
    auth_dir = tmp_path / "auths"
    auth_dir.mkdir(exist_ok=True)
    f = auth_dir / f"codex-{email}.json"
    f.write_text("{}", encoding="utf-8")
    return str(f)


def test_check_and_refresh_ignores_existing_auth_file_outside_auth_dir(tmp_path, monkeypatch):
    outside = tmp_path / "outside-auth.json"
    outside.write_text(json.dumps({"access_token": "outside-token"}), encoding="utf-8")

    quota_calls = []
    monkeypatch.setattr(manager, "check_codex_quota", lambda token, **_kw: quota_calls.append(token) or ("ok", {}))

    status, info = manager._check_and_refresh({"email": "unsafe@example.com", "auth_file": str(outside)})

    assert (status, info) == ("no_auth", None)
    assert quota_calls == []


def test_check_and_refresh_accepts_auth_file_inside_auth_dir(tmp_path, monkeypatch):
    auth_file = tmp_path / "auths" / "codex-safe@example.com-team-deadbeef.json"
    auth_file.write_text(
        json.dumps({"access_token": "inside-token", "account_id": "acc_123"}),
        encoding="utf-8",
    )

    quota_calls = []

    def fake_check(token, **kwargs):
        quota_calls.append((token, kwargs))
        return "ok", {"primary_pct": 1, "weekly_pct": 2}

    monkeypatch.setattr(manager, "check_codex_quota", fake_check)

    status, info = manager._check_and_refresh({"email": "safe@example.com", "auth_file": str(auth_file)})

    assert status == "ok"
    assert info == {"primary_pct": 1, "weekly_pct": 2}
    assert quota_calls == [("inside-token", {"account_id": "acc_123"})]


def test_check_and_refresh_treats_oversized_auth_file_as_no_auth(tmp_path, monkeypatch):
    auth_file = tmp_path / "auths" / "codex-huge@example.com-team-deadbeef.json"
    auth_file.write_text("x" * (AUTH_JSON_FILE_MAX_BYTES + 1), encoding="utf-8")

    quota_calls = []
    monkeypatch.setattr(manager, "check_codex_quota", lambda token, **_kw: quota_calls.append(token) or ("ok", {}))

    status, info = manager._check_and_refresh({"email": "huge@example.com", "auth_file": str(auth_file)})
...
def test_probe_treats_existing_auth_file_outside_auth_dir_as_no_auth(tmp_path, monkeypatch):
    outside = tmp_path / "outside-auth.json"
    outside.write_text(json.dumps({"access_token": "outside-token"}), encoding="utf-8")
    stby = {
    ...
    monkeypatch.setattr(
        manager,
        "_check_and_refresh",
        lambda _acc: pytest.fail("outside auth file should not be probed"),
    )
...
def test_check_codex_quota_classifies_windows_by_limit_seconds_and_keeps_plan(monkeypatch):
    ...
    assert info["primary_pct"] == 12
    assert info["primary_resets_at"] == 1785000000
    assert info["primary_window_seconds"] == 18000
    assert info["weekly_pct"] == 98
    assert info["weekly_resets_at"] == 1785551222
...
def test_check_codex_quota_does_not_show_monthly_window_as_5h_or_weekly(monkeypatch):
    ...
    assert info.get("primary_pct") is None
    assert info.get("weekly_pct") is None
    assert info["monthly_pct"] == 40
    assert info["monthly_resets_at"] == 1787585552
    assert info["monthly_window_seconds"] == 2592000
