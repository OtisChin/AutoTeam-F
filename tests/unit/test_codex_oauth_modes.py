...
def test_register_accounts_retries_retryable_skips_once_after_first_pass(monkeypatch):
    captured = {"calls": 0, "progress": []}
    ...
    def fake_create_account_direct(mail_client, **kwargs):
        ...
        email = f"new{captured['calls']}@example.com"
        kwargs["out_outcome"].update(status="success", email=email)
    ...
    result = manager.cmd_register_accounts(
        ...
        jitter_min_seconds=0,
        jitter_max_seconds=0,
        progress_callback=captured["progress"].append,
    )
    ...
    assert any(item.get("pending_retry") == 1 for item in captured["progress"])
    assert result["results"][1]["retry_round"] == 1
    assert result["results"][1]["first_attempt"]["status"] == "exception"


def test_register_accounts_does_not_retry_explicit_failures(monkeypatch):
    captured = {"calls": 0, "progress": []}
    ...
    def fake_create_account_direct(mail_client, **kwargs):
        captured["calls"] += 1
        kwargs["out_outcome"].update(status="phone_blocked", reason="add-phone")
        return None
    ...
    assert all(item.get("pending_retry") == 0 for item in captured["progress"])
