...
from autotoken.chatgpt_api import ChatGPTTeamAPI


def _make_api(monkeypatch):
    """构造一个不启动浏览器的 ChatGPTTeamAPI。"""
    monkeypatch.setattr("autotoken.chatgpt_api.get_chatgpt_account_id", lambda: "acct-test-1")
    monkeypatch.setattr("autotoken.chatgpt_api.get_chatgpt_workspace_name", lambda: "TestWS")
    api = ChatGPTTeamAPI()
    # retry sleep 直接短路,避免测试卡住 5s/15s
    monkeypatch.setattr("autotoken.chatgpt_api.time.sleep", lambda *_args, **_kw: None)
    return api
