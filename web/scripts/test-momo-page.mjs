...
assert.match(app, /momoVn: \(\) => import\('\.\/components\/MomoPage\.vue'\)/, 'App lazily registers the Momo page chunk')
assert.match(app, /gcashPh: \(\) => import\('\.\/components\/GCashPhPage\.vue'\)/, 'App lazily registers the GCash page chunk')
...
assert.match(app, /const MomoPage = shallowRef\(asyncPage\('momoVn'\)\)/, 'App creates the Momo async component')
assert.match(app, /const GCashPhPage = shallowRef\(asyncPage\('gcashPh'\)\)/, 'App creates the GCash async component')

assert.match(navigation, /key: 'momoVn'[\s\S]*label: '越南 MoMo'/, 'Shared navigation exposes 越南 MoMo entry')
assert.match(navigation, /key: 'momoVn'[\s\S]*key: 'gcashPh'[\s\S]*label: '菲律宾 GCash'/, 'Shared navigation keeps 菲律宾 GCash after 越南 MoMo')
