...
assert.match(page, /pay153LinkStatusFilter/, '153支付已提链账号支持按支付状态筛选')
assert.match(page, /togglePay153LinkSortOrder/, '153支付已提链账号支持切换顺序和倒序')
assert.match(page, /pay153QuickSelectCount/, '153支付已提链账号支持输入数量')
assert.match(page, /selectFirstPay153Accounts/, '153支付已提链账号支持快速勾选前N条')
assert.match(page, /pay153LinkAccountOptions = computed\(\(\) => filterPaymentLinkAccountsByStatus\([\s\S]*successfulPayPalLinkAccounts\(accounts\.value, links\.value, pay153LinkCountryFilter\.value, \{ timeFilter: pay153LinkTimeFilter\.value, sortOrder: pay153LinkSortOrder\.value \}\)[\s\S]*pay153LinkStatusFilter\.value[\s\S]*pay153PaymentAccountStatus/, '153支付链接列表只取未支付且有效期内，并支持时间、状态和顺序筛选')
...
assert.doesNotMatch(page, /togglePay153AutoPay" :disabled="pay153Busy && !pay153AutoPayActive"/, '153支付自动支付按钮在已有支付任务运行时仍可点击')
...
assert.match(page, /scanPay153AutoPayLinks[\s\S]*drainPay153AutoPayQueue/, '153支付自动扫描后会立即尝试启动队列')
assert.match(page, /pay153ManualOccupiedSlots/, '153支付自动队列会计算当前手动任务占用的并发槽')
assert.match(page, /drainPay153AutoPayQueue[\s\S]*availableSlots[\s\S]*launchPay153AutoPayItem/, '153支付自动队列按空闲并发槽立即补任务')
assert.match(page, /pollPay153Job[\s\S]*drainPay153AutoPayQueue/, '153支付手动任务结束后会继续启动自动队列')
