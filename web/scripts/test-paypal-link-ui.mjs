...
assert.match(page, /Access Token 池（优先于账号池）/, 'PayPal 提链页提供 access token 池')
assert.match(page, /const directAccessTokens = computed\(\(\) => parseAccessTokens\(form\.value\.accessTokens\)\)/, '前端会解析多行 access token')
assert.match(page, /const selectedAccessTokenItems = computed/, '前端维护可选择的 access token 池')
assert.match(page, /retryFailedAccessTokens/, 'access token 池支持失败重试')
assert.match(page, /syncAccessTokenPoolFromJob/, 'access token 池会从任务状态回写成功失败已支付')
assert.match(page, /accessTokenItems: accessTokenItems\.map/, '提链请求会提交带 label 的 accessTokenItems 字段')
assert.match(page, /PROTOCOL_BA_POOL_STORAGE_KEY/, '协议支付页会持久化手动 BA 链池')
assert.match(page, /PAY153_BA_POOL_STORAGE_KEY/, '153支付页会持久化手动 BA 链池')
assert.match(page, /const selectedProtocolBaItems = computed/, '协议支付页维护可选择的手动 BA 链池')
assert.match(page, /const selectedPay153BaItems = computed/, '153支付页维护可选择的手动 BA 链池')
assert.match(page, /retryFailedBaPool\('protocol'\)/, '协议支付 BA 链池支持失败重试')
assert.match(page, /retryFailedBaPool\('pay153'\)/, '153支付 BA 链池支持失败重试')
assert.match(page, /syncBaPoolFromJob\('protocol', job\)/, '协议支付任务会回写 BA 链池状态')
assert.match(page, /syncBaPoolFromJob\('pay153', job\)/, '153支付任务会回写 BA 链池状态')
