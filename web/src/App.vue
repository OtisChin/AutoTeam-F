    ...
    <main class="workspace-shell">
      <div class="workspace-chrome">
        <div class="workspace-main">
          ...
          <MailAccountsPage v-else-if="currentPage === 'mailAccounts'" />
...
const PAGE_KEYS = new Set(['dashboard', 'register', 'cardpool', 'bindcard', 'gopay', 'gopayPro', 'paypal', 'paypalIce', 'oauthPhones', 'oauthPhoneRecords', 'mailAccounts', 'trade', 'cpa2sub', 'oauth', 'tasks', 'logs', 'settings'])
...
function taskStageLabel(stage) {
  return {
    ...
    paypal_pending_retry_wait: 'PayPal 待重试等待',
    paypal_pending_retry_started: 'PayPal 待重试开始',
    paypal_pending_retry_queued: 'PayPal 加入待重试池',
    paypal_pending_retry_account: 'PayPal 正在重试账号',
    paypal_pending_retry_failed: 'PayPal 待重试失败',
    binding: '绑卡中',
    completed: '流程完成',
    failed: '流程失败',
  }[String(stage || '')] || ''
