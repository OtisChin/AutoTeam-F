...
async function request(method, path, body = null, options = {}) {
  ...
  const resp = await fetchWithTimeout(`${BASE}${path}`, opts, options)
  let data
  try {
...
export const api = {
  ...
  getAccounts: (options = {}) => {
    ...
    const requestOptions = Number.isFinite(options.timeoutMs) ? { timeoutMs: options.timeoutMs } : {}
    if (includeSessionStubs === undefined) return request('GET', '/accounts', null, requestOptions)
    return request('GET', `/accounts?include_session_stubs=${includeSessionStubs ? 'true' : 'false'}`, null, requestOptions)
  },
  ...
  getIdealQrBlob: async (value) => {
    ...
    const resp = await fetchWithTimeout('/api/ideal/qr', {
      method: 'POST',
      headers,
