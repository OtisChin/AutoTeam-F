<template>
  <div class="space-y-5">
    <WorkflowWorkspace title="GCash 提链" eyebrow="支付 / Philippines" description="按配置、启动、进度和结果组织业务操作" :status-label="workflowStatusPresentation(busy ? 'running' : 'success').label" :status-tone="workflowStatusPresentation(busy ? 'running' : 'success').tone">
    ...
    <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5 md:p-6">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Philippines Wallet 任务</p>
          <h2 class="mt-1 text-2xl font-bold text-white">菲律宾 GCash 提链</h2>
          <p class="mt-2 text-sm text-gray-400">在账号池中勾选一个或多个账号执行 PH/PHP 的 GCash 提链；可只检测资格，也可完整提链。</p>
        </div>
    ...
    <div class="grid grid-cols-1 items-start gap-5 2xl:grid-cols-[minmax(360px,0.85fr)_minmax(460px,1.1fr)_minmax(420px,0.9fr)]">
      <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5">
        <div class="border-b border-gray-800 pb-4">
          ...
          <h3 class="mt-1 text-xl font-bold text-white">PH 代理</h3>
        </div>

        <div class="mt-5 space-y-5">
          <label class="block">
            <span class="mb-2 block text-sm font-semibold text-gray-300">PH 代理列表</span>
            <textarea
              v-model.trim="form.proxies"
              rows="8"
              spellcheck="false"
              placeholder="每行一个代理；支持 host:port:user-region-PH-sid-xxx-t-120:pass 或 socks5h://user:pass@host:port"
            ...
            ></textarea>
            <span class="mt-1 block text-xs text-gray-500">1024/ArxLabs 的 host:port:user:pass 会自动按 socks5h 使用；建议全程使用 PH 地区代理。</span>
          </label>
      ...
      <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5">
        ...
        <div class="mt-4 flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <button
              @click="deleteSelectedGCashAccounts"
              :disabled="inputLocked || deletingGCashAccounts.size > 0 || !selectedEmails.length"
            ...
            <tbody class="divide-y divide-gray-900">
              ...
              <tr v-for="account in visibleAccounts" :key="account.email" class="hover:bg-gray-900/50">
                ...
                <td class="px-3 py-2 text-right">
                  <button
                    @click="deleteGCashAccount(account.email)"
                    :disabled="inputLocked || deletingGCashAccounts.has(account.email)"
                    ...
                    title="从 GCash 账号池和仪表盘账号池中删除该账号"
                  >
                    {{ deletingGCashAccounts.has(account.email) ? '删除中' : '删除' }}
                  </button>
                </td>
              </tr>
            </tbody>
          ...
          <div v-else class="mt-5 max-h-72 space-y-3 overflow-y-auto pr-1 text-sm">
            ...
            <div v-if="recentResultFilter !== 'failed'" v-for="item in visibleRecentResultSuccesses" :key="item.email" class="rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-100">
              ...
              <div class="mt-2 flex flex-wrap gap-2">
                ...
                <button @click="copy(gcashLinkUrl(item.link))" :disabled="!gcashLinkUrl(item.link)" class="rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-emerald-100 disabled:opacity-50">复制链</button>
    ...
    <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5">
      <div class="flex flex-col gap-3 border-b border-gray-800 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h3 class="mt-1 text-xl font-bold text-white">已提取 GCash 链接</h3>
        </div>
        <div class="flex flex-wrap gap-2">
        ...
        <table class="min-w-[1380px] w-full text-left text-sm">
          <thead class="sticky top-0 bg-gray-900 text-xs uppercase tracking-wide text-gray-500">
            <tr>
              ...
              <th class="px-3 py-2">二维码</th>
              <th class="px-3 py-2">GCash 链接</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-900">
            ...
            <tr v-for="link in visibleLinks" :key="link.id" class="hover:bg-gray-900/50">
              ...
              <td class="px-3 py-2 text-xs text-gray-400">{{ gcashLinkAmountLabel(link) }}</td>
              <td class="px-3 py-2 font-mono text-xs text-gray-400">{{ link.cs_id || '-' }}</td>
              <td class="whitespace-nowrap px-3 py-2 text-xs">
                <span class="rounded-full border px-2 py-1 font-semibold" :class="gcashExpiryClass(link)">
                  {{ gcashExpiryText(link) }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="flex flex-wrap gap-2">
                  <a :href="gcashLinkUrl(link) || '#'" target="_blank" rel="noopener" class="rounded border border-blue-500/30 bg-blue-500/10 px-2 py-1 text-xs text-blue-200" :class="!gcashLinkActionable(link) ? 'pointer-events-none opacity-50' : ''">打开</a>
                  <button @click="copy(gcashLinkUrl(link))" :disabled="!gcashLinkActionable(link)" class="rounded border border-gray-700 bg-gray-900 px-2 py-1 text-xs text-gray-200 disabled:opacity-50">复制链</button>
                  <button @click="copy(gcashQrValue(link))" :disabled="!gcashQrValue(link)" class="rounded border border-cyan-500/30 bg-cyan-500/10 px-2 py-1 text-xs text-cyan-100 disabled:opacity-50">复制码</button>
                </div>
              </td>
              <td class="max-w-[260px] truncate px-3 py-2 font-mono text-xs text-gray-500">{{ gcashQrValue(link) || '-' }}</td>
              <td class="max-w-[360px] truncate px-3 py-2 font-mono text-xs text-gray-500">{{ gcashLinkUrl(link) || '-' }}</td>
            </tr>
          </tbody>
        </table>
...
const JOB_STORAGE_KEY = 'autotoken_gcash_ph_job'
const TERMINAL_STATUSES = new Set(['success', 'error', 'failed', 'cancelled'])
const GCASH_LINK_TTL_MS = 10 * 60 * 1000
...
const statusText = ref('请选择账号并填写 PH 代理后开始提链。')
...
function successNotificationKey(item, index) {
  ...
  const link = gcashLinkUrl(item?.link || item)
...
function accountSelectable(account) {
  return accountStatus(account) !== 'paid' && account?.gcash_selectable !== false
}
...
function accountStatus(account) {
  return accountJobStatus(account)?.status || account?.gcash_status || 'pending'
}

function isOaicsUnsupportedGCashError(message) {
  const text = String(message || '').trim().toLowerCase()
  return text.includes('openai_custom_checkout_unsupported')
    || text.includes('oaics checkout cannot use stripe payment_pages gcash flow')
}
...
function accountStatusText(account) {
  ...
  if (status === 'failed' && isOaicsUnsupportedGCashError(accountStatusError(account))) return '提链失败（oaics 当前不支持）'
  if (status === 'failed') return '提链失败'
  return '未提链'
}

function accountStatusError(account) {
  return accountJobStatus(account)?.error || account?.gcash_error || ''
}

function recentResultErrorText(item) {
  const error = String(item?.error || '').trim()
  if (isOaicsUnsupportedGCashError(error)) return '提链失败（oaics 当前不支持）'
  return error || '提链失败'
}
...
function gcashLinkUrl(link) {
  return String(link?.provider_redirect_url || link?.gcash_link || link?.stripe_redirect_url || '').trim()
}

function gcashQrValue(link) {
  return String(link?.gcash_qr_data || link?.gcash_qr_url || link?.gcash_qr_image || '').trim()
}

function gcashLinkCurrency(link) {
  return String(link?.currency || link?.billing?.currency || 'PHP').trim().toUpperCase() || 'PHP'
}

function gcashLinkAmountLabel(link) {
  const amount = String(link?.amount ?? '').trim()
  return `${amount || '-'} ${gcashLinkCurrency(link)}`
}
...
function gcashExpiresAtMs(link) {
  const explicit = timestampMs(link?.gcash_expires_at_ts ?? link?.gcashExpiresAtTs ?? link?.gcash_expires_at ?? link?.gcashExpiresAt)
  if (explicit) return explicit
  const created = timestampMs(link?.created_at_ts ?? link?.createdAtTs ?? link?.created_at ?? link?.createdAt)
  return created ? created + GCASH_LINK_TTL_MS : 0
}

function gcashRemainingMs(link) {
  const expiresAt = gcashExpiresAtMs(link)
  return expiresAt ? expiresAt - nowMs.value : 0
}

function gcashLinkExpired(link) {
  const expiresAt = gcashExpiresAtMs(link)
  return Boolean(expiresAt && expiresAt <= nowMs.value)
}

function gcashLinkActionable(link) {
  return Boolean(gcashLinkUrl(link)) && !gcashLinkExpired(link)
}

function gcashExpiryText(link) {
  if (!gcashLinkUrl(link)) return '-'
  const expiresAt = gcashExpiresAtMs(link)
...
function gcashExpiryClass(link) {
  if (!gcashLinkUrl(link)) return 'border-gray-700 bg-gray-900 text-gray-400'
  if (gcashLinkExpired(link)) return 'border-rose-500/30 bg-rose-500/10 text-rose-300'
  if (gcashRemainingMs(link) <= 60 * 1000) return 'border-amber-500/30 bg-amber-500/10 text-amber-300'
...
async function refreshAccounts() {
  const accountData = await api.getGCashPhAccounts()
...
async function refreshLinks() {
  const linkData = await api.getGCashPhLinks()
...
function saveProxy() {
  saveForm()
  setStatus('GCash PH 代理配置已保存到本地浏览器。')
}

async function deleteGCashAccount(email) {
  const target = String(email || '').trim()
  if (!target) return
  if (!window.confirm(`确认从 GCash 账号池和仪表盘账号池中删除 ${target}？`)) return
  const nextDeleting = new Set(deletingGCashAccounts.value)
  nextDeleting.add(target)
  deletingGCashAccounts.value = nextDeleting
  try {
    const data = await api.deleteGCashPhAccount(target)
    const next = new Set(selectedAccounts.value)
    next.delete(target)
    selectedAccounts.value = next
    const gcash = data.gcash_ph || data.gcash || {}
    setStatus(`已删除账号 ${target}：仪表盘账号 ${data.dashboard_account_deleted ? '已删除' : '未找到'}，认证 ${data.auth_session_deleted ? '已删除' : '未找到'}，GCash 链接 ${gcash.links_deleted || 0} 条。`)
    await reloadAll()
  } catch (error) {
    setStatus(`删除账号失败：${cleanError(error)}`, true)
  } finally {
    const done = new Set(deletingGCashAccounts.value)
    done.delete(target)
    deletingGCashAccounts.value = done
  }
}

async function deleteSelectedGCashAccounts() {
  const emails = selectedEmails.value.map(email => String(email || '').trim()).filter(Boolean)
  if (!emails.length) return
  if (!window.confirm(`确认批量删除选中的 ${emails.length} 个账号？这些账号会同时从 GCash 账号池和仪表盘账号池删除。`)) return
  deletingGCashAccounts.value = new Set(emails)
  try {
    const data = await api.deleteGCashPhAccounts(emails)
    ...
    const linkCount = (data.results || []).reduce((sum, item) => sum + Number((item.gcash_ph || item.gcash || {}).links_deleted || 0), 0)
    setStatus(`已批量删除 ${data.deleted || deleted.size} 个账号，清理 GCash 链接 ${linkCount} 条。`)
    await reloadAll()
  } catch (error) {
    setStatus(`批量删除账号失败：${cleanError(error)}`, true)
  } finally {
    deletingGCashAccounts.value = new Set()
  }
}
...
async function deleteSelectedLinks() {
  ...
  const data = await api.deleteGCashPhLinks(ids)
...
async function clearLinks() {
  if (!links.value.length || !window.confirm('确认清空所有 GCash 链接？')) return
  const data = await api.clearGCashPhLinks()
  links.value = Array.isArray(data.links) ? data.links : []
  selectedLinkIds.value = new Set()
...
function exportLinks() {
  ...
  a.href = url
  a.download = `gcash-ph-links-${new Date().toISOString().slice(0, 10)}.json`
  a.click()
