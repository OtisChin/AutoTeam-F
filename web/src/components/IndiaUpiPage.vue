  ...
  <div class="space-y-5">
    <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5 md:p-6">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p class="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">独立 UPI 任务</p>
    ...
    <div class="grid grid-cols-1 gap-5 2xl:grid-cols-[minmax(360px,0.85fr)_minmax(460px,1.1fr)_minmax(420px,0.9fr)]">
      <section class="rounded-2xl border border-gray-800 bg-gray-950/70 p-5">
        <div class="border-b border-gray-800 pb-4">
          ...
          <h3 class="mt-1 text-xl font-bold text-white">IN 代理</h3>
        </div>
...
async function startWithEmails(emails, actionText = '提取') {
  ...
  setStatus(`任务已提交，正在为 ${accountEmails.length} 个账号${actionText} UPI，并发 ${form.value.concurrency}。`)
  try {
    ...
    const data = await api.startIndiaUpiBatch({ ...payload, accountEmails })
