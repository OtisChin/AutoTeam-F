<template>
  <div class="space-y-5">
    <WorkflowWorkspace title="印度 UPI" eyebrow="支付 / India" description="按配置、启动、进度和结果组织业务操作" :status-label="workflowStatusPresentation(busy ? 'running' : 'success').label" :status-tone="workflowStatusPresentation(busy ? 'running' : 'success').tone">
    <UiSegmentedControl v-model="activeUpiTab" :options="[{ value: 'extract', label: '提链' },{ value: 'tempExtract', label: '临时提链' },{ value: 'payment', label: '支付' }]" aria-label="印度 UPI模式" />
    ...
    <section v-if="activeUpiTab === 'payment'" class="overflow-hidden rounded-2xl border border-cyan-500/20 ui-workflow-surface p-5 shadow-2xl shadow-black/30 md:p-6">
